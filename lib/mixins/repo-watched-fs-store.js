...
function mixin(repo, fs) {
  ...
  repo.initWatcher = function() {
    ...
    .on('change', function(path, root, stat) {
      console.log(`Watcher received change event for file: ${path} repoPath: ${repo.path}`, stat.mtime);
      if (stat.isDirectory()) return
      repo.updateFilePaths()
      var file = repo.getFile(path) || path;
      // DOING: Only read the file if modifiedTime is later than file object
