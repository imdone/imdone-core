  ...
  repo.initWatcher = function (cb) {
    ...
    repo.watcher = sane(repo.path, {
      ...
      ignored(file) {
        ...
        if (!initializing && _isImdoneConfig(relPath)) {
          onConfigChange(relPath)
        }

        if (!initializing && _isImdoneIgnore(relPath)) {
          onIgnoreChange(relPath)
        }
        const ignore = !repo.shouldInclude(relPath)
        if (!ignore || /^.*\.(jpg|gif|pdf|png)$/i.test(relPath)) {
          repo.addFilePath(relPath)
        }
        return ignore
      },
    })
