  ...
  repo.initWatcher = function (cb) {
    ...
    repo.watcher = sane(repo.path, {
      ...
      ignored(file) {
        ...
        const isImage = /^.*\.(jpg|gif|pdf|png)$/i.test(file)
        if (isImage) {
          console.log('Excluding image file from watcher:', file)
          return true
        }
        const include = repo.shouldInclude(file)
        if (include) {
          console.log('Including file in watcher:', file)
