...
export function watchedFileSystemStoreMixin(repo, fs = realFs) {
  ...
  function trackWatcherEvent(listener) {
    return (...args) => {
      const event = Promise.resolve().then(() => listener(...args))
      const trackedEvent = event.catch((err) => {
        logger.error('Error processing watcher event')
        logger.error(err)
        repo.emit('watch.error', err)
      })
      watcherEvents.add(trackedEvent)
      void trackedEvent.finally(() => watcherEvents.delete(trackedEvent))
    }
  }
  ...
  repo.destroy = async function () {
    ...
    await Promise.allSettled([...watcherEvents])
  ...
  repo.initWatcher = async () => {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      repo.watcher = chokidar.watch(repo.path, {
        ...
        .on('add', trackWatcherEvent(async function (path, stat) {
          logger.log('Watcher received add event for file: ' + path)
          if (stat.isDirectory()) return
          const relativePath = repo.getRelativePath(path)
          let file = repo.getFile(relativePath)
          if (file === undefined) {
          ...
          stat = await repo.fileOK(relativePath)
          if (!stat) return
        ...
        .on('change', trackWatcherEvent(async function (path, stat) {
          logger.log(
          ...
          if (!initializing && _isImdoneConfig(relPath)) return onConfigChange(relPath)
          if (!initializing && _isImdoneIgnore(relPath)) return onIgnoreChange(relPath)
          ...
          const ok = await repo.fileOK(file?.path || path)
          if (!ok) return
          log('Reading file: ' + path)
          if (!isFile) {
            const relativePath = repo.getRelativePath(path)
            file = repo.getFile(relativePath) || new File({
              repoId: repo.getId(),
              filePath: relativePath,
              languages: repo.languages,
              project: repo.project,
        ...
        .on('unlink', trackWatcherEvent(async function (path) {
          log('Watcher received unlink event for file: ' + path)
          const relativePath = repo.getRelativePath(path)
          const file = repo.getFile(relativePath) || new File({
            repoId: repo.getId(),
            filePath: relativePath,
            languages: repo.languages,
            project: repo.project,
          })
          repo.removeFilePath(relativePath)
          log('Removing file: ' + relativePath)
