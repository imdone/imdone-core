...
function mixin(repo, fs) {
  ...
  repo.loadConfig = function(cb) {
    // #DONE:20 Fix loadConfig defaults
    // DOING: issue:2 assigned:piascikj If a config is bad move it to config.json.bak and save a new one with defaults +enhancement
    cb = tools.cb(cb);
  ...
  // #ARCHIVE:0 Allow (all) param to not check ignores
  ...
  // #ARCHIVE:60 Make getFileTree async
