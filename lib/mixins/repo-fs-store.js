...
function mixin(repo, fs) {
  ...
  function ignore({path, stat}) {
    const ignore = _ignore().add(DEFAULT_IGNORE_DIRS);
    if (stat.isFile() && !ignore.ignores(path)) repo.allFiles.push(path)
    return !repo.shouldExclude(path) && !stat.isSymbolicLink();
  }

  function ls (root, cb) {
    if (root === repo.path) repo.allFiles = []
