...
function mixin(repo, fs) {
  ...
  repo.loadConfig = function(cb) {
    // #ARCHIVE: Fix loadConfig defaults
    // TODO: assigned:piascikj If a config is bad move it to config.json.bak and save a new one with defaults +enhancement +standup gh-iss:2
    cb = tools.cb(cb);
    repo.loadIgnore();
  ...
  // #ARCHIVE: Allow (all) param to not check ignores
  ...
  // #ARCHIVE: Make getFileTree async
