...
function mixin(repo, fs) {
  ...
  repo.loadConfig = function(cb) {
    ...
    // #DOING:0 git-iss:2 assigned:piascikj If a config is bad move it to config.json.bak and save a new one with defaults +enhancement +standup
    cb = tools.cb(cb);
    repo.loadIgnore();
  ...
  // #ARCHIVE:40 Allow (all) param to not check ignores
  ...
  // #ARCHIVE:100 Make getFileTree async
