...
function mixin(repo, fs = require('fs')) {
  ...
  repo.addFilePath = function (path) {
    if (!repo.allFiles.includes(path)) {
    ...
    _init.call(repo, (err) => {
      ...
      fs.stat(repo.path, function (err, stat) {
        if (err || !stat.isDirectory()) {
          var error = new Error(
            'Path must be an existing directory on the file system'
          )
  ...
  repo.updateConfig = function (loadedConfig, cb) {
    ...
    if (loadedConfig && loadedConfig.sync)
      loadedConfig.sync = {
        ...loadedConfig.sync,
        useImdoneioForPriority: false,
      }
    ...
    if (!/\.\.(\/|\\)/.test(file.path)) {
      ...
      if (fs.existsSync(dirName)) {
        if (
          ...
          repo.config.journalType === constants.JOURNAL_TYPE.NEW_FILE
        ) {
          return repo.deleteFile(file.path, cb)
        }
        return write()
      }
