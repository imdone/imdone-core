...
function mixin(repo, fs) {
  ...
  repo.loadConfig = function(cb) {
    // #DONE:10 Fix loadConfig defaults
    // #DOING:10 If a config is bad move it to config.json.bak and save a new one with defaults
    cb = tools.cb(cb);
    if (repo.config) cb(null, new Config(_.defaults(repo.config, constants.DEFAULT_CONFIG)));
    else {
      var file = repo.getFullPath(CONFIG_FILE);
      fs.exists(file, function(exists) {
        if (exists) {
          fs.readFile(file, function(err, data) {
            ...
            cb(null, new Config(_.defaults(configLike, constants.DEFAULT_CONFIG)));
          });
        } else cb(null, new Config(constants.DEFAULT_CONFIG));
      });
    }
  };
      ...
      _.each(allPaths, function(path) {
        ...
        repo.fileOK(path, includeDirs, function(err, stat) {
          ...
          if (stat) {
            ...
            var file = new File(repo.getId(), path, undefined, stat.mtime);
  ...
  // #DOING:20 Allow (all) param to not check ignores
  ...
  // #DONE:80 Make getFileTree async
