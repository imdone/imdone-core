...
function mixin(repo, fs = require('fs')) {
  ...
  repo.fileOK = function(file, includeDirs, cb) {
    ...
    cb = tools.cb(cb);
    try {
      if (!file || this.shouldExclude(file)) return cb(null, false);
    } catch (e) {
      cb(e)
    }
  ...
  repo.updateConfig = function(loadedConfig, cb) {
    ...
    const newConfig = new Config(_assign({}, baseConfig, repo.config, loadedConfig));
  ...
  repo.getTemplates = function(qry, cb) {
    ...
    // BACKLOG:-680 Implement getTemplates id:39 gh:143 ic:gh
    //  - [ ] Test
  };
