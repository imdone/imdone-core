...
function mixin(repo, fs) {
  ...
  repo.fileOK = function(file, includeDirs, cb) {
    ...
    fs.stat(fullPath, function(err, stat) {
      ...
      if (stat.isFile()) {
        var chunk = readChunk.sync(fullPath, 0, 256);
        if (isBinaryFile.sync(chunk, 256)) return cb(null, false);
        if (fileType(chunk)) return cb(null, false);
        cb(null, stat);
  ...
  repo.loadConfig = function(cb) {
    // #ARCHIVE:50 Fix loadConfig defaults id:14
    // TODO:40 assigned:piascikj If a config is bad move it to config.json.bak and save a new one with defaults +enhancement +standup gh-iss:2 id:15
    cb = tools.cb(cb);
    repo.loadIgnore();
  ...
  repo.writeFile = function(file, emitFileUpdate, cb) {
    ...
    if (!/\.\.(\/|\\)/.test(file.path)) {
      ...
      var write = function() {
        if (!file.content) return cb("File has no content");
  ...
  // #ARCHIVE:70 Allow (all) param to not check ignores id:16
  ...
  // #ARCHIVE:130 Make getFileTree async id:17
