...
function mixin(repo, fs) {
  ...
  repo.loadConfig = function(cb) {
    cb = tools.cb(cb);
    if (repo.config) cb(null, new Config(_.defaults(_.cloneDeep(constants.DEFAULT_CONFIG, repo.config))));
    else {
      var file = repo.getFullPath(CONFIG_FILE);
      fs.exists(file, function(exists) {
        if (exists) {
          fs.readFile(file, function(err, data) {
            ...
            cb(null, new Config(_.defaults(_.cloneDeep(constants.DEFAULT_CONFIG, configLike))));
          });
