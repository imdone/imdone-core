...
function mixin(repo, fs) {
  ...
  repo.removeFilePath = function (path) {
    _.remove(repo.allFiles, item => item === path)
  }
  ...
  repo.findFileForTopic = function (topic) {
    const fileRegex = new RegExp(`(${topic})(\\.md)?$`)
    const foundFile = repo.getFilePaths().find(path => fileRegex.test(path))
    return foundFile ? _path.join(repoPath, foundFile) : undefined
  }
