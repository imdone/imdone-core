...
function mixin(repo, fs) {
  ...
  repo.init = function(cb) {
    ...
    fs.stat(repo.path, function(err, stat) {
      ...
      repo.loadConfig(function(err, config, isNew) {
        if (err) {
          repo.emit('error', err);
          repo.initializing = false;
          return cb(err);
  ...
  repo.addInitialSort = function (isNew, cb) {
    if (!isNew) return cb()
  ...
  repo.loadIgnore = function() {
    ...
    let patterns = fs.existsSync(ignoreFile) ? fs.readFileSync(ignoreFile).toString() : ''
    if (fs.existsSync(gitIgnoreFile)) patterns = fs.readFileSync(gitIgnoreFile).toString() + "\n" + patterns
  ...
  repo.loadConfig = function(cb) {
    // TODO: If a config is bad move it to config.json.bak and save a new one with defaults gh:2 id:9 +enhancement +standup +chore
    cb = tools.cb(cb);
    repo.loadIgnore();
    var file = repo.getFullPath(CONFIG_FILE);
