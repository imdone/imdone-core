...
export function buildMigrateConfig (repo, defaultSettings = _defaultSettings) {
  ...
  return function (config = repo.config) {
    ...
    if (config && config.settings) {
      ...
      const defaultCardsSettings = cloneDeep(defaultSettingsCopy.cards)
      const cardsSettings = cloneDeep(config.settings.cards)

      delete defaultSettingsCopy.cards
      delete config.settings.cards

      config.settings = { ...defaultSettingsCopy, ...config.settings }
      config.settings.cards = {
