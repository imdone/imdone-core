...
module.exports = class CliBacklogPlugin extends Plugin {
  ...
  getStoryProjectBoardActions() {
    ...
    const groupActions = [{name: 'All tasks', value: '*'}, ...groups].map(({name, value}) => {
    ...
    const storyId = project.getAllCards('meta.story-id = *')[0].meta['story-id'][0]
    const storiesDir = _path.dirname(project.path)
    const backlogDir = _path.dirname(storiesDir)
    const showStoryAction = {
      name: 'Show story',
      action: function () {
        const query = encodeURIComponent(`allMeta.story-id="${storyId}"`)
        project.openUrl(`imdone://${backlogDir}?filter=${query}`) // imdone:///Users/jesse/projects/imdone-projects/imdone-core/backlog?filter=allMeta.story-id="Board-action-to-show-backlog-from-story-project"
      }
    }

    return [showStoryAction, ...groupActions]
  }
