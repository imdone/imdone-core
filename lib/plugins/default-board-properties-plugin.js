...
module.exports = class DefaultBoardPropertiesPlugin extends Plugin {
  ...
  // READY Also include templates (md files) in .github and it's subdirs 
  // <!--
  // order:0
  // -->
  async getTemplates() {
    const { preparePathForWriting, readdir, readFile, stat } = this.project.fileGateway;
    ...
    const templatesFromGithubDir = await readMarkdownFiles(githubDir, 'github_');
