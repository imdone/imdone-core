...
export class PluginManager extends Emitter {
  constructor(project) {
    ...
    this.defaultPlugins = [
      ...
      'epic-plugin.js', 
      'extension-plugin.js',
    ].map((plugin) => _path.join(__dirname, plugin))
  ...
  async loadPlugin(path) {
    ...
    const pathWithDate = `${_path.resolve(path)}?t=${Date.now()}`

    try {
      // eslint-disable-next-line
      delete require.cache[require.resolve(fullPath)]
      // eslint-disable-next-line
      const pluginClass = require(_path.resolve(path))
