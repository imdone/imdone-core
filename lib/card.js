...
module.exports = function newCard(task, _project, dontParse) {
  ...
  class Card extends Task {
    ...
    initActions () {
      this.links = this.getCardActions()
    ...
    initData () {
      ...
      const pluginData = pluginManager ? { ...pluginManager.getCardProperties(cardData), ...cardData} : cardData
      const extensionData = {
        ...project.config.cardPropertiesFunction(pluginData), 
        ...pluginData
      }
      const data =  {
        ...extensionData,
