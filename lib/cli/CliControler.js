...
CliController.startTask = async function (projectPath = defaultProjectPath, taskId, log) {
  ...
  const branch = await CliController.git(process.env.PWD).branchLocal()
  log('currentBranch', branch.current)
  if (branch.current !== branchName) {
    await CliController.git(process.env.PWD).checkoutBranch(branchName, 'HEAD')
  }
}
