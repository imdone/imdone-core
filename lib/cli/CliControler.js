...
CliController.imdoneInit = async function ({projectPath = defaultProjectPath, configPath = DEFAULT_CONFIG_PATH, tasksDir = STORIES_DIR}) {
  ...
  const backlogProject = await new BacklogProject(projectPath, config).init()
  return backlogProject.project
}

CliController.addTask = async function ({task, projectPath = defaultProjectPath, list, tags, contexts, log}) {

  const project = await new BacklogProject(projectPath).init().project
...
CliController.importMarkdown =  async function(projectPath = defaultProjectPath, markdown, log) {
  ...
  storyId = project.sanitizeFileName(storyId)
...
CliController.startTask = async function (projectPath = defaultProjectPath, taskId, log) {
  ...
  const project = await new BacklogProject(projectPath, null, ApplicationContext);
  await project.init()
...
async function createStoryProject(projectPath, storyTitle, log) {
  const storyProjectPath = getStoryProjectPath(projectPath, storyTitle)
  try {
