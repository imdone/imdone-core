...
class BacklogProject {
  ...
  async addStory(content, storyId) {
    const storyPath = path.join(this.getStoryProjectPath(storyId), 'README.md')
    const file = await this.addTaskToFile({ path: storyPath, list: TODO, content })
  ...
  getStoryIds() {
    return [
      ...new Set(
        this.project.getCards(`meta.${STORY_ID}=* AND tags=story`).map(({meta, text}) => ({text, storyId: meta[STORY_ID][0]}))
      )
    ]
  }
