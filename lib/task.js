...
class Task {
  ...
  getTextAndDescription () {
    ...
    return `${this.markdownBeforeText}${text}${lineEnd}${descriptionMD.join(lineEnd)}`.replace(/<!--\s*\[\s*([\s\S]*?)\s*\]\s*-->/gm, '$1')
  }
...
Task.getLinePos = function(content, lineNo) {
  ...
  while ((result = re.exec(normalizedContent)) !== null) {
    if (line == lineNo) {
      return (Task.hasWindowsEOL(content)) ? result.index + line-1 : result.index;
    }
...
Task.prototype.removeFromContent = function(content, toRemove) {
  let modifiedContent = content.replace(toRemove + lineEnd, '')
...
Task.prototype.updateOrderMeta = function(config, newLine) {
  let descContent = this.description.join(lineEnd)
  ...
  this.description = Task.padDescription(descContent.split(lineEnd), this.beforeText)
}
