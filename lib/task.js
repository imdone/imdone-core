...
Task.prototype.parseDueDate = function() {
  if (this.getDueMeta()) return
  const match = this.text.match(/due\s.*?\./i)
  if (!match) return
  const due = chrono.parseDate(match[0])
  try {
    console.log('Found due date:', due)
    const dueString = due.toISOString()
    this.addMetaData('due', dueString)
    this.due = due
  } catch (e) {
    console.log(`Problem parsing due date for ${this.list} ${this.text} in ${this.source.path}`)
  }
}
