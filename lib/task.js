...
class Task {
  ...
  getTextAndDescription () {
    ...
    const descriptionMD = description.map(ln => ln.trim().startsWith('-') ? ln : ln.trim()).join(os.EOL)
    const sep = `  ${os.EOL}` // (description.length > 0 && (!description[0].trim() || description[0].trim().startsWith('-'))) ? `${os.EOL}  ${os.EOL}` : ' '
    return `${text}${sep}${descriptionMD}`
  }
...
Task.prototype.parseTodoTxt = function() {
  // #BACKLOG: #hashtags should be treated like todo.txt tags gh:78 id:4 +enhancement
...
Task.prototype.updateTags = function() {
  // TODO:120 implement updateTags id:7 gh:93
  if (this.tags) {
...
Task.prototype.updateContext = function() {
  // TODO:110 implement updateContext id:1 gh:124
  var self = this;
  if (this.context) {
    let text = this.text.replace(new RegExp(Task.ContextRegExp), "")
