...
Task.eachMetaInContent = function(content, cb) {
  ...
  Task.execMetaRegex(new RegExp(Task.MetaRegExp), content, codePositions, cb)
  Task.execMetaRegex(new RegExp(Task.MetaRegExpQuotes), content, codePositions, cb)
}
...
Task.addToLastCommentInContent = function(content, toAdd, newLine) {
  ...
  const lineNo = Task.getLineNumber(content, index)
  const startOfLine = Task.getLinePos(content, lineNo)
  const paddingLength = index - startOfLine
  if (newLine) prefix = prefix.padEnd(paddingLength + 1)
  console.log({lineNo, startOfLine, index, paddingLength})
  if (lastIndex > 0) {
    let before = content.slice(0, lastIndex - 3).trimEnd()
    // if (before.endsWith(lf)) before = before.slice(0, before.length - 1)
    const after = content.slice(lastIndex - 3)
    return `${before}${prefix}${toAdd}${prefix}${after}`
  }
  if (content !== '') content = content + lf
  return `${content}<!--${prefix}${toAdd}${prefix}-->${lf}`
}

Task.removeFromContent = function(content, toRemove) {
...
Task.prototype.updateOrderMeta = function(newLine) {
  let descContent = this.description.join(lf)
  if (!this.order) return
  if (!this.meta.order) {
    descContent = Task.addToLastCommentInContent(descContent, `order:${this.order}`, newLine)
  } else {
    const regex = new RegExp(Task.ORDER_META_REGEX)
    descContent = descContent.replace(regex, `order:${this.order}`)
  }
  this.meta.order = [this.order]
  this.description = descContent.split(lf)
}
...
Task.removeMetaDataFromText = function (text) {
  const re = new RegExp(Task.MetaRegExp)
  const re2 = new RegExp(Task.MetaRegExpQuotes)
