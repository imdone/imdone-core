...
Task.prototype.getHtml = function(opts) {
  ...
  if (!this.description || this.description.length === 0) return taskHtml;
  let descHtml = marked(this.description.join(os.EOL));
  const $ = cheerio.load(descHtml);
  $('ul').find("input[type='checkbox']").closest('ul').addClass('checklist');
  return `${taskHtml}${os.EOL}${os.EOL}${$.html()}`;
};
