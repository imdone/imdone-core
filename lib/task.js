...
var _clone = require('lodash.clone'),
  ...
  { extractWikilinkTopics } = require('./adapters/markdown');
...
const {
  ...
  isNumber
} = require('./adapters/parsers/task/CardContentParser.js')
...
class Task {
  ...
  updateFromContent(content) {
    const terminatorCount = getBlankLineTerminatorCountFromConfig(this.config)
    ...
    content = content.replace(excessiveBlankLinePattern, replacement)
...
const DATE_META_KEYS = Task.DATE_META_KEYS = ['due', 'defer', 'created', 'started', 'remind', 'completed', 'archivedAt']
...
Task.trimDescription = function (rawTaskContentLines, beforeText) {
  const checkedData = getCheckedData(beforeText)
  if (checkedData) {
...
Task.getMarkdownLinkLabelPositions = function (text) {
  const positions = [];
  const re = /\[([^\]]+)\]\([^\)]+\)/g;
  let match;
...
Task.isResultInMarkdownLinkLabel = function (linkLabelPositions, index) {
...
Task.prototype.updateOrderMeta = function (config, descContent = this.descriptionString) {
  if (config.orderMeta) {
