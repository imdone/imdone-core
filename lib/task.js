...
Task.prototype.getCheckedData = function() {
  ...
  const result = taskRegex.exec(this.beforeText)
  return result 
    ? {
      pad: result[1].length, 
      checked: result[2] === Task.CHECKED
    }
    : null
}
...
Task.prototype.addToLastCommentInContent = function(content, toAdd, newLine) {
  ...
  const paddingLength = checkedData ? checkedData.pad : 0
...
Task.prototype.removeFromContent = function(content, toRemove) {
  // TODO respect padding of task prefix for list items
  const checkedData = this.getCheckedData()
  if (checkedData) toRemove = toRemove.padStart(checkedData.pad + toRemove.length)
...
Task.prototype.updateOrderMeta = function(newLine) {
  ...
  if (!this.meta.order) {
    descContent = this.addToLastCommentInContent(descContent, `order:${this.order}`, newLine)
  } else {
