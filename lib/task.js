...
class Task {
  ...
  getIssueNumbersFromDesc () {
    ...
    if (!issueURL) {
      const match = this.description.find(desc => this.issueNumberRegExp().test(desc))
      if (match) issueURL = match.match(this.issueNumberRegExp())
    }
    if (!issueURL) return numbers
    numbers.push(issueURL[0].split('/').pop())
    return numbers
  }

  getIssueNumbers (idMetaKey) {
    let numbers = []
    if (this.hasIssueURL()) {
      numbers = this.getIssueNumbersFromDesc()
    } else {
      numbers = this.meta[idMetaKey] || [];
    }
    return numbers
  }
