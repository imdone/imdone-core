...
export class Task {
  ...
  getTextAndDescription() {
    ...
    .replace(/<!--\s*\[\s*([\s\S]*?)\s*\]\s*-->/gm, '$1') // Safe: simple capture group, no backtracking issue
    return this.getText(
      {
        stripMeta: true,
        sanitize: true,
  ...
  getCommentTagsOnly () {
    return this.config?.getCommentTagsOnly()
  }
  ...
  static addToLastCommentInContent (content, toAdd, newLine) {
    ...
    // Find last comment using state machine (safe from backtracking)
    let lastIndex = -1;
      ...
      if (content[i] === '<' && content[i + 1] === '!' && content[i + 2] === '-' && content[i + 3] === '-') {
        ...
        while (i < len - 2) {
          if (content[i] === '-' && content[i + 1] === '-' && content[i + 2] === '>') {
            i += 3;
            lastIndex = i; // Remember end of this comment
            break;
          }
          i++;
        }
  ...
  static getTags (text, prefix = '+', commentTagsOnly = false) {
    const tags = [];
    const codePositions = Task.getMarkdownCodePositions(text);
    const linkLabelPositions = Task.getMarkdownLinkLabelPositions(text);
    const commentPositions = Task.getCommentPositions(text);
    let result;
    const re = Task.getTagRegexp(prefix);

    while ((result = re.exec(text)) !== null) {
      if (commentTagsOnly && !Task.isResultInComment(commentPositions, result.index)) continue;
  ...
  static parseMetaData (config, content) {
    const commentMetaOnly = config.getCommentMetaOnly()
    const commentPositions = Task.getCommentPositions(content);
    var meta = {}
    Task.eachMetaInContent(config, content, (result) => {
      if (
        commentMetaOnly 
        && !Task.isResultInComment(commentPositions, result.index)
      ) return;;
  ...
  static getMetaData (config, task) {
    const text = Task.isTask(task) ? task.getRawTextAndDescription() : task

    const meta = Task.parseMetaData(config, text)
  ...
  static getCommentPositions (text, useRegex = false) {
    if (useRegex) {
      return Task._getCommentPositionsRegex(text);
    }

    // State machine implementation (safe from catastrophic backtracking)
    const positions = [];
    const len = text.length;
    let i = 0;

    while (i < len - 3) { // Need at least 4 chars for <!--
      // Look for comment start <!--
  ...
  static _getCommentPositionsRegex (text) {
    // OLD IMPLEMENTATION - DO NOT USE IN PRODUCTION
    // This regex can cause catastrophic backtracking with nested/malformed comments
    const positions = [];
    const re = /<!--[\s\S]*?-->/g;
    let match;
  ...
  static isResultInComment (commentPositions, index) {
    return commentPositions.some(pos => index >= pos.start && index < pos.end);
  }
  ...
  static getContext (text, commentTagsOnly = false) {
    const context = []
    const commentPositions = Task.getCommentPositions(text);
    const codePositions = Task.getMarkdownCodePositions(text)
    var result,
  ...
  static getMarkdownCodePositions (text, useRegex = false) {
    if (useRegex) {
      return Task._getMarkdownCodePositionsRegex(text);
    }
  ...
  static _getMarkdownCodePositionsRegex (text) {
    // OLD IMPLEMENTATION - DO NOT USE IN PRODUCTION
  ...
  parseTodoTxt () {
    this.tags = Task.getTags(this.getRawTextAndDescription(), this.getTagPrefix(), this.getCommentTagsOnly())
    this.context = Task.getContext(this.getRawTextAndDescription(), this.getCommentTagsOnly())
