...
class Task {
  constructor(config, obj, dontParse) {
    ...
    this.parseTodoTxt()
    if (this.meta.order) {
      this.order = parseFloat(this.meta.order)
    }
  ...
  getOrder () {
    return _isNumber(this.order) ? this.order : ''
  }
  ...
  getRawTextAndDescription () {
    ...
    return `${text}${lf}${descriptionMD}${lf}`
  }
...
Task.padDescription = function(description = [], beforeText = '') {
  ...
  if (checkedData) {
    description = description.map(line => {
      if (line.trimStart().length !== line.length) return line
      return line.padStart(line.length + checkedData.pad)
    })
  }
  return description
}
...
Task.Types = {
  CODE: "CODE",
  HASHTAG: "HASHTAG",
  HASH_META_ORDER:"HASH_META_ORDER",
  MARKDOWN: "MARKDOWN"
};

Task.AnyLanguageGroup = '\\w:%/.$-\\u1FD3\\u1FD6-\\u1FDB\\u1FE0-\\u1FEC\\u1FF2-\\u1FF4\\u1FF6-\\u1FFC\\u2071\\u207F\\u2090-\\u209C\\u2102\\u2107\\u210A-\\u2113\\u2115\\u2119-\\u211D\\u2124\\u2126\\u2128\\u212A-\\u212D\\u212F-\\u2139\\u213C-\\u213F\\u2145-\\u2149\\u214E\\u2183\\u2184\\u2C00-\\u2C2E\\u2C30-\\u2C5E\\u2C60-\\u2CE4\\u2CEB-\\u2CEE\\u2CF2\\u2CF3\\u2D00-\\u2D25\\u2D27\\u2D2D\\u2D30-\\u2D67\\u2D6F\\u2D80-\\u2D96\\u2DA0-\\u2DA6\\u2DA8-\\u2DAE\\u2DB0-\\u2DB6\\u2DB8-\\u2DBE\\u2DC0-\\u2DC6\\u2DC8-\\u2DCE\\u2DD0-\\u2DD6\\u2DD8-\\u2DDE\\u2E2F\\u3005\\u3006\\u3031-\\u3035\\u303B\\u303C\\u3041-\\u3096\\u309D-\\u309F\\u30A1-\\u30FA\\u30FC-\\u30FF\\u3105-\\u312D\\u3131-\\u318E\\u31A0-\\u31BA\\u31F0-\\u31FF\\u3400-\\u4DB5\\u4E00-\\u9FCC\\uA000-\\uA48C\\uA4D0-\\uA4FD\\uA500-\\uA60C\\uA610-\\uA61F\\uA62A\\uA62B\\uA640-\\uA66E\\uA67F-\\uA697\\uA6A0-\\uA6E5\\uA717-\\uA71F\\uA722-\\uA788\\uA78B-\\uA78E\\uA790-\\uA793\\uA7A0-\\uA7AA\\uA7F8-\\uA801\\uA803-\\uA805\\uA807-\\uA80A\\uA80C-\\uA822\\uA840-\\uA873\\uA882-\\uA8B3\\uA8F2-\\uA8F7\\uA8FB\\uA90A-\\uA925\\uA930-\\uA946\\uA960-\\uA97C\\uA984-\\uA9B2\\uA9CF\\uAA00-\\uAA28\\uAA40-\\uAA42\\uAA44-\\uAA4B\\uAA60-\\uAA76\\uAA7A\\uAA80-\\uAAAF\\uAAB1\\uAAB5\\uAAB6\\uAAB9-\\uAABD\\uAAC0\\uAAC2\\uAADB-\\uAADD\\uAAE0-\\uAAEA\\uAAF2-\\uAAF4\\uAB01-\\uAB06\\uAB09-\\uAB0E\\uAB11-\\uAB16\\uAB20-\\uAB26\\uAB28-\\uAB2E\\uABC0-\\uABE2\\uAC00-\\uD7A3\\uD7B0-\\uD7C6\\uD7CB-\\uD7FB\\uF900-\\uFA6D\\uFA70-\\uFAD9\\uFB00-\\uFB06\\uFB13-\\uFB17\\uFB1D\\uFB1F-\\uFB28\\uFB2A-\\uFB36\\uFB38-\\uFB3C\\uFB3E\\uFB40\\uFB41\\uFB43\\uFB44\\uFB46-\\uFBB1\\uFBD3-\\uFD3D\\uFD50-\\uFD8F\\uFD92-\\uFDC7\\uFDF0-\\uFDFB\\uFE70-\\uFE74\\uFE76-\\uFEFC\\uFF21-\\uFF3A\\uFF41-\\uFF5A\\uFF66-\\uFFBE\\uFFC2-\\uFFC7\\uFFCA-\\uFFCF\\uFFD2-\\uFFD7\\uFFDA-\\uFFDC'
Task.EmojiGroup = '\\p{Emoji_Presentation}'

Task.TagsRegExp = new RegExp(`(^|\\s)[+#]([${Task.AnyLanguageGroup}${Task.EmojiGroup}]+)`, 'gmiu')
Task.getTagRegexp = function(prefix = '+') {
  return new RegExp(`(^|\\s)[${prefix}]([${Task.AnyLanguageGroup}${Task.EmojiGroup}]+)`, 'gmiu')
}
Task.getTags = function(text, prefix = '+') {
  const tags = []
  var codePositions = Task.getMarkdownCodePositions(text)
  var result, re = Task.getTagRegexp(prefix)
...
Task.ContextRegExp = new RegExp(`(^|\\s)@([${Task.AnyLanguageGroup}${Task.EmojiGroup}]+)`, 'gmiu')
...
Task.MetaRegExp = new RegExp(`(^|\\s)([a-zA-Z-_]+?):(?!\\/\\/)([${Task.AnyLanguageGroup}${Task.EmojiGroup}]+)`, 'gu');
Task.getMetaRegExp = function(config) {
  return new RegExp(`(^|\\s)([a-zA-Z-_]+?)${config.getMetaSep()}(?!\\/\\/)([${Task.AnyLanguageGroup}${Task.EmojiGroup}]+)`, 'gu')
}
Task.MetaRegExpQuotes = new RegExp(`(^|\\s)([a-zA-Z-_]+?):"((?:""|[^"]|[${Task.AnyLanguageGroup}${Task.EmojiGroup}])*)"`, 'gu')
Task.getMetaRegExpQuotes = function(config) {
  return new RegExp(`(^|\\s)([a-zA-Z-_]+?)${config.getMetaSep()}"((?${config.getMetaSep()}""|[^"]|[${Task.AnyLanguageGroup}${Task.EmojiGroup}])*)"`, 'gu')
}

Task.getMetaData = function(config, task) {
  var text = Task.isTask(task) ? task.getRawTextAndDescription() : task
  var meta = {};
  Task.eachMetaInContent(config, text, result => {
    var key = result[2];
    var val = result[3];
...
Task.eachMetaInContent = function(config, content, cb) {
  ...
  Task.execMetaRegex(Task.getMetaRegExpQuotes(config), content, codePositions, cb)
}
...
Task.prototype.getTagPrefix = function() {
  return _get(this, 'config.settings.tagPrefix', '+')
}

Task.prototype.addToLastCommentInContent = function(content, toAdd, newLine) {
  // TODO respect padding of task prefix for list items
  let prefix = newLine ? lf : ' '
  ...
  if (!content.endsWith(lf) && content !== '') content = content + lf
  return `${content}<!--${prefix}${toAdd}${prefix}-->`
}

Task.prototype.removeFromContent = function(content, toRemove) {
  let modifiedContent = content.replace(toRemove + lf, '')
  if (modifiedContent == content) {
    modifiedContent = content.replace(toRemove, '')
  }
  return modifiedContent
}

Task.ORDER_META_REGEX = /order:("(-?[\d.]+(?:e-?\d+)?)"|(-?[\d.]+(?:e-?\d+)?))/gm
Task.prototype.updateOrderMeta = function(config, newLine) {
  let descContent = this.description.join(lf)
  if (!this.order) return
  if (!this.meta.order) {
    descContent = this.addToLastCommentInContent(descContent, `order${config.getMetaSep()}${this.order}`, newLine)
  } else {
    const regex = new RegExp(Task.ORDER_META_REGEX)
    descContent = descContent.replace(regex, `order${config.getMetaSep()}${this.order}`)
  }
  this.meta.order = [this.order]
  this.description = Task.padDescription(descContent.split(lf), this.beforeText)
}

Task.prototype.replaceContent = function(regex, replacement) {
  this.text = this.text.replace(regex, replacement)
  this.description = this.description.map(line => line.replace(regex, replacement))
}
 ...
 * @return text
 */
// TODO make this use config.getMetaSep
