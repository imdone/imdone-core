...
Task.addToLastCommentInContent = function(content, toAdd, newLine) {
  ...
  if (lastIndex > 0) {
    ...
    if (prefix === lf && before.endsWith(lf)) before = before.slice(0, before.length -1)
    const after = content.slice(lastIndex - 3)
    return `${before}${prefix}${toAdd}${prefix}${after}`
  }
  return `${content}${prefix}<!--${prefix}${toAdd}${prefix}-->${prefix}`
}

Task.ORDER_META_REGEX = /order:("(-?[\d.]+(?:e-?\d+)?)"|(-?[\d.]+(?:e-?\d+)?))/gm
Task.prototype.updateOrderMeta = function() {
  let descContent = this.description.join(lf)
  if (!this.order) return
  if (!this.meta.order) {
    descContent = Task.addToLastCommentInContent(descContent, `order:${this.order}`, true)
  } else {
