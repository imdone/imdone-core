...
class Task {
  ...
  getTextAndDescription () {
    ...
    const descriptionMD = description.map(ln => ln.trim().startsWith('-') ? ln : ln.trimEnd()).join(lf)
...
Task.prototype.updateTags = function() {
  ...
  if (this.tags) {
    ...
    let text = this.text.replace(new RegExp(Task.TagsRegExp), (match, p1, p2, offset) => {
...
Task.prototype.updateContext = function() {
  ...
  if (this.context) {
    const positions = Task.getMarkdownCodePositions(this.text)
    let text = this.text.replace(new RegExp(Task.ContextRegExp), (match, p1, p2, offset) => {
      if (Task.isResultInMarkdownCode(positions, offset)) return match
      return ' '
    }).replace(/\s+/g, " ").trim()
...
Task.prototype.updateMetaData = function() {
  ...
  const positions = Task.getMarkdownCodePositions(this.text)
  this.text = this.text.replace(re, (match, p1, p2, offset) => {
    if (Task.isResultInMarkdownCode(positions, offset)) return match
    return ''
  }).replace(/\s+/g, " ").trim()
  var meta = this.meta
...
Task.prototype.getText = function(opts, text) {
  ...
  if (opts) {
    if (opts.stripMeta) {
      ...
      text = this.text
      text = text.replace(new RegExp(Task.MetaRegExp), (match, p1, p2, offset) => {
        if (Task.isResultInMarkdownCode(positions, offset)) return match
        return ''
      }).replace(/\s+/g, " ").trim()
    }
    if (opts.stripTags) {
      const positions = Task.getMarkdownCodePositions(text)
      text = text.replace(new RegExp(Task.TagsRegExp), (match, p1, p2, offset) => {
        if (Task.isResultInMarkdownCode(positions, offset)) return match
        return ''
      }).replace(/\s+/g, " ").trim()
    ...
    if (opts.stripContext) {
      const positions = Task.getMarkdownCodePositions(text)
      text = text.replace(new RegExp(Task.ContextRegExp), (match, p1, p2, offset) => {
        if (Task.isResultInMarkdownCode(positions, offset)) return match
        return ''
      }).replace(/\s+/g, " ").trim()
...
Task.prototype.getDescription = function(opts) {
  return this.description.map(ln => {
    let line = ln.trim().startsWith('-') ? ln : ln.trimEnd()
