...
function removeDisplayComments(description) {
  ...
  logger.debug(`[INSTRUMENT] removeDisplayComments START - contentLength: ${description?.length || 0}`);
  ...
  while (i < len) {
    ...
    if (iterations % 10000 === 0) {
      logger.debug(`[INSTRUMENT] removeDisplayComments - iteration ${iterations}, position ${i}/${len}`);
    }
  ...
  logger.debug(`[INSTRUMENT] removeDisplayComments END - elapsed: ${elapsed}ms, iterations: ${iterations}`);
  return result;
}
...
export function interpolate(content, data, opts = { tags: ['${', '}'] }) {
  const startTime = Date.now();
  logger.debug(`[INSTRUMENT] interpolate START - contentLength: ${content?.length || 0}, tags: ${JSON.stringify(opts.tags)}`);
  ...
  while (i < len) {
    iterations++;
    if (iterations % 10000 === 0) {
      logger.debug(`[INSTRUMENT] interpolate - iteration ${iterations}, position ${i}/${len}, elapsed: ${Date.now() - startTime}ms`);
    }
  ...
  const elapsed = Date.now() - startTime;
  logger.debug(`[INSTRUMENT] interpolate END - elapsed: ${elapsed}ms, iterations: ${iterations}, hasInterpolation: ${hasInterpolation}`);
...
export function format(content, data, mustache) {
  ...
  logger.debug(`[INSTRUMENT] format START - contentLength: ${content?.length || 0}, mustache: ${mustache}`);

  const errorTokens = {};

  if (mustache) {
    logger.debug(`[INSTRUMENT] format - calling removeDisplayComments`);
    content = removeDisplayComments(content);
  }

  // Escape code blocks before processing interpolation
  logger.debug(`[INSTRUMENT] format - calling escapeCodeBlocks`);
  const escapeStart = Date.now();
  const { content: escapedContent, codeBlocks } = escapeCodeBlocks(content);
  logger.debug(`[INSTRUMENT] format - escapeCodeBlocks completed in ${Date.now() - escapeStart}ms`);

  let result = escapedContent;
  let hasInterpolation = true;
  let interpolationRound = 0;

  while (hasInterpolation) {
    interpolationRound++;
    logger.debug(`[INSTRUMENT] format - interpolation round ${interpolationRound} with tags ['${'${'}', '}']`);
  ...
  if (mustache) {
    hasInterpolation = true;
    interpolationRound = 0;
    while (hasInterpolation) {
      interpolationRound++;
      logger.debug(`[INSTRUMENT] format - mustache interpolation round ${interpolationRound} with tags ['{{', '}}']`);
      const interpolationResult = interpolate(result, data, { tags: ['{{', '}}'] });
      result = interpolationResult.content;
      hasInterpolation = interpolationResult.hasInterpolation;
      Object.assign(errorTokens, interpolationResult.errorTokens);
  ...
  logger.debug(`[INSTRUMENT] format - calling restoreCodeBlocks`);
  ...
  const elapsed = Date.now() - formatStart;
  logger.debug(`[INSTRUMENT] format END - elapsed: ${elapsed}ms`);
  return result;
}
