...
function removeDisplayComments(description) {
  ...
  console.log(`[TRACE] removeDisplayComments START - contentLength: ${description?.length || 0}`);
  ...
  while (i < len) {
    ...
    if (iterations % 10000 === 0) {
      console.log(`[TRACE] removeDisplayComments - iteration ${iterations}, position ${i}/${len}`);
    }
  ...
  console.log(`[TRACE] removeDisplayComments END - elapsed: ${elapsed}ms, iterations: ${iterations}`);
  return result;
}
...
export function interpolate(content, data, opts = { tags: ['${', '}'] }) {
  const startTime = Date.now();
  console.log(`[TRACE] interpolate START - contentLength: ${content?.length || 0}, tags: ${JSON.stringify(opts.tags)}`);
  ...
  while (i < len) {
    iterations++;
    if (iterations % 10000 === 0) {
      console.log(`[TRACE] interpolate - iteration ${iterations}, position ${i}/${len}, elapsed: ${Date.now() - startTime}ms`);
    }
  ...
  const elapsed = Date.now() - startTime;
  console.log(`[TRACE] interpolate END - elapsed: ${elapsed}ms, iterations: ${iterations}, hasInterpolation: ${hasInterpolation}`);
...
export function format(content, data, mustache) {
  ...
  console.log(`[TRACE] format START - contentLength: ${content?.length || 0}, mustache: ${mustache}`);

  const errorTokens = {};

  if (mustache) {
    console.log(`[TRACE] format - calling removeDisplayComments`);
    content = removeDisplayComments(content);
  }

  // Escape code blocks before processing interpolation
  console.log(`[TRACE] format - calling escapeCodeBlocks`);
  const escapeStart = Date.now();
  const { content: escapedContent, codeBlocks } = escapeCodeBlocks(content);
  console.log(`[TRACE] format - escapeCodeBlocks completed in ${Date.now() - escapeStart}ms`);
  ...
  while (hasInterpolation) {
    interpolationRound++;
    console.log(`[TRACE] format - interpolation round ${interpolationRound} with tags ['${'${'}', '}']`);
  ...
  if (mustache) {
    ...
    while (hasInterpolation) {
      interpolationRound++;
      console.log(`[TRACE] format - mustache interpolation round ${interpolationRound} with tags ['{{', '}}']`);
      const interpolationResult = interpolate(result, data, { tags: ['{{', '}}'] });
      result = interpolationResult.content;
      hasInterpolation = interpolationResult.hasInterpolation;
      Object.assign(errorTokens, interpolationResult.errorTokens);
  ...
  console.log(`[TRACE] format - calling restoreCodeBlocks`);
  ...
  console.log(`[TRACE] format END - elapsed: ${elapsed}ms`);
  return result;
}
