...
function removeDisplayComments(description) {
  ...
  logger.debug(`[TRACE] removeDisplayComments START - contentLength: ${description?.length || 0}`);
  ...
  while (i < len) {
    ...
    if (iterations % 10000 === 0) {
      logger.debug(`[TRACE] removeDisplayComments - iteration ${iterations}, position ${i}/${len}`);
    }
  ...
  logger.debug(`[TRACE] removeDisplayComments END - elapsed: ${elapsed}ms, iterations: ${iterations}`);
  return result;
}
...
export function interpolate(content, data, opts = { tags: ['${', '}'] }) {
  const startTime = Date.now();
  logger.debug(`[TRACE] interpolate START - contentLength: ${content?.length || 0}, tags: ${JSON.stringify(opts.tags)}`);
  ...
  while (i < len) {
    iterations++;
    if (iterations % 10000 === 0) {
      logger.debug(`[TRACE] interpolate - iteration ${iterations}, position ${i}/${len}, elapsed: ${Date.now() - startTime}ms`);
    }
  ...
  const elapsed = Date.now() - startTime;
  logger.debug(`[TRACE] interpolate END - elapsed: ${elapsed}ms, iterations: ${iterations}, hasInterpolation: ${hasInterpolation}`);
...
export function format(content, data, mustache) {
  ...
  logger.debug(`[TRACE] format START - contentLength: ${content?.length || 0}, mustache: ${mustache}`);

  const errorTokens = {};

  if (mustache) {
    logger.debug(`[TRACE] format - calling removeDisplayComments`);
    content = removeDisplayComments(content);
  }

  // Escape code blocks before processing interpolation
  logger.debug(`[TRACE] format - calling escapeCodeBlocks`);
  const escapeStart = Date.now();
  const { content: escapedContent, codeBlocks } = escapeCodeBlocks(content);
  logger.debug(`[TRACE] format - escapeCodeBlocks completed in ${Date.now() - escapeStart}ms`);
  ...
  while (hasInterpolation) {
    interpolationRound++;
    logger.debug(`[TRACE] format - interpolation round ${interpolationRound} with tags ['${'${'}', '}']`);
  ...
  if (mustache) {
    ...
    while (hasInterpolation) {
      interpolationRound++;
      logger.debug(`[TRACE] format - mustache interpolation round ${interpolationRound} with tags ['{{', '}}']`);
      const interpolationResult = interpolate(result, data, { tags: ['{{', '}}'] });
      result = interpolationResult.content;
      hasInterpolation = interpolationResult.hasInterpolation;
      Object.assign(errorTokens, interpolationResult.errorTokens);
  ...
  logger.debug(`[TRACE] format - calling restoreCodeBlocks`);
  ...
  logger.debug(`[TRACE] format END - elapsed: ${elapsed}ms`);
  return result;
}
