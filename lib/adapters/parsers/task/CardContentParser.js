...
function isCustomCardTerminator(config, line) {
  if (!config.customCardTerminator) return false

  const isStringTerminator = typeof config.customCardTerminator === 'string'
    && config.customCardTerminator.trim()

  if (isStringTerminator) {
    const terminatorMatches = line && line.trim() === config.customCardTerminator.trim()
    return terminatorMatches
  }
...
function getBlankLineTerminatorCount(config) {
  const hasNumericTerminator = typeof config.customCardTerminator === 'number'
...
function getTextFileTaskContent ({
  ...
  let openBlockCommentTokens = 0
  let closeBlockCommentTokens = 0
  const blankLineTerminatorCount = getBlankLineTerminatorCount(config)
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (thisLineBlank) {
      ...
      if (consecutiveBlankLines >= blankLineTerminatorCount) {
        trailingBlankLines = blankLineTerminatorCount
        break
      }
    } else {
  ...
  const reachedEndNaturally = trailingBlankLines === 0
  const hasTrailingBlankLines = consecutiveBlankLines > 0
  const isUsingDefaultTerminator = blankLineTerminatorCount === DEFAULT_BLANK_LINE_TERMINATOR_COUNT

  if (reachedEndNaturally && hasTrailingBlankLines && isUsingDefaultTerminator) {
    trailingBlankLines = consecutiveBlankLines
  }
