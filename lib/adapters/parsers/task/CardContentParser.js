...
function getTaskContent ({
  ...
  const isWrappedWithCardTag =
    isMarkDownFile && lines.length > 1 && lines[1].trim() === START_TAG
  for (let i = isWrappedWithCardTag ? 2 : 1; i < lines.length; i++) {
    let line = lines[i]
    // Check for end of task
    // DOING: Make each of these a function for clarity
    if (isCheckBoxTask(config, line, beforeText) && !isWrappedWithCardTag)
      break
    ...
    if (isWrappedWithCardTag || line.trim() !== '') commentLines.push(line)
  }
  ...
  if (
    ...
    commentLines.pop()
  return { rawDescription: commentLines, isWrappedWithCardTag, hasCardEndTag }
}
