...
describe('Content Transformer', () => {
  ...
  describe('State machine safety - catastrophic backtracking prevention', () => {
    it('should handle content with many dollar signs without hanging', () => {
      // This would cause catastrophic backtracking with the old regex [\s\S]+?
      const content = '$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$';
      const data = {};
      const result = format(content, data, false);
    ...
    it('should handle content with many opening braces without hanging', () => {
      const content = '${{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{';
      const data = {};
      const result = format(content, data, false);
      expect(result).to.equal(content);
    ...
    it('should handle unclosed template variables', () => {
      const content = 'Start ${variable more content ${another';
      const data = { variable: 'value', another: 'test' };
      const result = format(content, data, false);
      // Unclosed variables should be left as-is
      expect(result).to.equal(content);
    });

    it('should handle nested curly braces in template content', () => {
      const content = 'Test ${obj.nested.value} end';
      const data = { obj: { nested: { value: 'result' } } };
      const result = format(content, data, false);
      expect(result).to.equal('Test result end');
    });

    it('should handle display comments with many brackets without hanging', () => {
      const content = '<!-- [ [[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[ ] -->';
      const data = {};
      const result = format(content, data, true);
