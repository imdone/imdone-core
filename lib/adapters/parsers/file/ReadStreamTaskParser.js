...
export class ReadStreamTaskParser {
  constructor(readStream) {
    ...
    this.lineIterator = this.readInterface[Symbol.asyncIterator]()
  }
  ...
  onLineRead() {
    // Hook for parsers that need line-aware state.
  }
  ...
  getTaskFromLine(line) {
    if (!this.shouldParseTaskLine(line)) return
    const originalLine = line
    line = this.getLineForTaskParsing(line)
    let task
    this.taskParsers.find((taskParser) => {
      task = taskParser.parseLine(line, this.lineNo, this.task, originalLine)
      return task
    })
    return task
  }
