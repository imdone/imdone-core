...
function obsidianTopicLinks(filePath) {
  return function(md) {
    ...
    function obsidianLinkRenderer(state, silent) {
      ...
      const href = encodeURI(resolveFileUrl(topic, filePath));

      // Create a new token
      const token = state.push("obsidian_link", "", 0);
      token.content = `<a href="${href}">${md.utils.escapeHtml(alias)}</a>`;
  ...
  return function(md) {
    ...
    md.renderer.rules.image = function(tokens, idx, options, env, self) {
      ...
      if (srcIndex >= 0) {
        ...
        const srcUrl = isFileLink(src) && resolveFileUrl(src, filePath)
...
function isFileLink(href) {
  return !/^(http|https|mailto):/.test(href)
}
