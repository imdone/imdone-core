...
module.exports = class EpicPlugin extends Plugin {
  ...
  onBoardUpdate (lists) {
    ...
    console.log(`${this.project.name} epics found:`,Object.keys(this.epics).length)
  ...
  updateCardMD (lists) {
    this.epicNames.forEach(epicId => {
      ...
      if (epic.list === this.project.doneList && (this.isListHidden(epic.list, lists) || this.isListIgnored(epic.list, lists))) return

      const epicText = removeMD(epic.text)
      const epicFilter = encodeURIComponent(`(allMeta.epic="${epicId}" or allMeta.is-epic="${epicId}")`)
      const epicFilePath = encodeURI(`${epic.source.repoId}/${epic.source.path}`.replace(/\\/g, '/'))
      const epicUrl = `imdone://${epicFilePath}?line=${epic.line}&filter=${epicFilter}`
      const epicLink = `**Epic:** [${epicText}](${epicUrl})\n\n`
      
      const taskList = epicLists.length === 0 
        ? '*None*\n' 
