...
File.prototype.newTask = function({
    ...
    // DOING:10 add beforeText for code files
...
File.addCompleted = function({task, content, config}) {
  ...
  content = Task.addToLastMetaInContent(content, `completed:${now}`)
  const lines = eol.split(content)
  return lines.join(lf)
}

File.prototype.updateMetaData = function (lists) {
  this.getTasks().forEach(task => {
    if (task.hasListChanged(lists)) {
      const now = (new Date()).toISOString()
      Task.addToLastMetaInContent(content, `${task.list}:${now}`)
    }
  })
}
