...
File.prototype.newTask = function(config, list, text, order, line, type, hasColon, content) {
  ...
  const description = self.isCodeFile() ? rawDescription.map(line => self.trimCommentChars(line)) : rawDescription
  var task = new Task({
    text: text,
    list: list,
    rawDescription,
...
File.prototype.modifyDescription = function(task) {
  ...
  const description = task.description.map(desc => `${spaces}${this.getLang().symbol} ${desc}`).join(os.EOL)+os.EOL
