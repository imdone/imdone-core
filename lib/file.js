...
File.prototype.getLang = function() {
  // #DONE:0 Allow a project to extend languages by adding them to config.json git-iss:5
...
// #DONE:10 This is not working on some files (Created in windows)
...
File.prototype.modifyCodeOrHashTask = function(re, task) {
  ...
  if (result) {
    ...
    if (task.id == getTaskId(self.getPath(), line, taskText)) {
      ...
      var taskContent = util.format("#%s:%d %s", task.list, task.order, text);
...
// [Implement #listname style tasks in any text file](#ARCHIVE:130)
// [Allow code style tasks to be included on list name](#ARCHIVE:120)
/**
 * Description
 * @method extractTasks
 * @return ThisExpression
 */
// #ARCHIVE:30 Have a seperate extract and modify method for code files and only look in comments and block comments
