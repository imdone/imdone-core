...
export class File extends Emitter {
  ...
  extractTasks (config) {
    ...
    log(`[INSTRUMENT] File.extractTasks START for ${this.path}`)

    this.tasks = []
    if (this.isMarkDownFile()) {
      const data = this.parseFrontMatter(config)
      if (data.imdone_ignore || data['kanban-plugin']) {
        log(`[INSTRUMENT] File.extractTasks END (ignored) - ${Date.now() - startTime}ms`)
        return this
      }
    }

    const frontMatterTime = Date.now()
    log(`[INSTRUMENT] Front matter check took ${frontMatterTime - startTime}ms`)

    if (this.isCodeFile()) {
      log(`[INSTRUMENT] About to extractTasksInCodeFile`)
      this.extractTasksInCodeFile(config)
      log(`[INSTRUMENT] extractTasksInCodeFile took ${Date.now() - frontMatterTime}ms`)
    } else {
      log(`[INSTRUMENT] About to extractTasksInNonCodeFile`)
      this.extractTasksInNonCodeFile(config)
      log(`[INSTRUMENT] extractTasksInNonCodeFile took ${Date.now() - frontMatterTime}ms`)
    }

    const totalTime = Date.now() - startTime
    if (totalTime > 1000) {
      log(`[INSTRUMENT] ⚠️  SLOW File.extractTasks: ${this.path} took ${totalTime}ms`)
    }
    log(`[INSTRUMENT] File.extractTasks END - total ${totalTime}ms`)
  ...
  extractTasksInCodeFile (config) {
    // INSTRUMENTATION: Track regex iterations
    const startTime = Date.now()
    var self = this
    var commentRegex = this.getCodeCommentRegex()
    log(`[INSTRUMENT] getCodeCommentRegex took ${Date.now() - startTime}ms`)

    var result
    var iterationCount = 0
    const contentLength = self.getContent()?.length || 0
    log(`[INSTRUMENT] Starting regex loop, content length: ${contentLength}`)

    while ((result = commentRegex.exec(self.getContent())) !== null) {
      iterationCount++
      if (iterationCount % 100 === 0) {
        log(`[INSTRUMENT] Regex iteration ${iterationCount}, elapsed: ${Date.now() - startTime}ms`)
      }
      ...
      if (iterationCount > 10000) {
        log(`[INSTRUMENT] ⚠️  WARNING: Exceeded 10000 regex iterations, breaking loop`)
        break
      }
    }

    const totalTime = Date.now() - startTime
    log(`[INSTRUMENT] extractTasksInCodeFile END - ${iterationCount} iterations, ${totalTime}ms`)
  }

  extractTasksInNonCodeFile (config) {
    // INSTRUMENTATION: Track each extraction method
    const startTime = Date.now()
    const content = this.getContent()
    const contentLength = content?.length || 0
    log(`[INSTRUMENT] extractTasksInNonCodeFile START, content length: ${contentLength}`)

    log(`[INSTRUMENT] About to extractHashStyleTasks`)
    this.extractHashStyleTasks(config, 0, content)
    const hashTime = Date.now()
    log(`[INSTRUMENT] extractHashStyleTasks took ${hashTime - startTime}ms`)

    log(`[INSTRUMENT] About to extractLinkStyleTasks`)
    this.extractLinkStyleTasks(config, 0, content)
    const linkTime = Date.now()
    log(`[INSTRUMENT] extractLinkStyleTasks took ${linkTime - hashTime}ms`)

    log(`[INSTRUMENT] About to extractCheckboxTasks`)
    this.extractCheckboxTasks(config, 0, content)
    const checkboxTime = Date.now()
    log(`[INSTRUMENT] extractCheckboxTasks took ${checkboxTime - linkTime}ms`)

    const totalTime = Date.now() - startTime
    log(`[INSTRUMENT] extractTasksInNonCodeFile END - total ${totalTime}ms`)
  }
  ...
  modifyCodeOrHashTask (re, task, config, noEmit) {
    ...
    while ((result = re.exec(this.getContent())) !== null) {
      ...
      if (this.tasksMatch(config, task, line, taskText)) {
        ...
        var taskContent = getRawTask({tokenPrefix, orderMeta: config.orderMeta, hasColon: task.hasColon, list: task.list, order: task.order, text, type: task.type})
  ...
  getCodeCommentRegex () {
    ...
    if (lang.block) {
      ...
      // Use [^] instead of (.|[\r\n]) and remove lazy quantifier for better performance
      // [^] matches any character including newlines without the overhead of alternation
      // Using greedy match with proper end delimiter is safer than lazy quantifier
      reString = reString + '|(?<!["\'`])' + start + '[^]*?' + end
    }
  ...
  parseFrontMatter (config) {
    ...
    try {
      ...
      logger.warn(`Error processing front-matter in:${this.path}`, err)
    }
    return {}
  }
  ...
  _getMarkdownCodePositionsRegex (text) {
