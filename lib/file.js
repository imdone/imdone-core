...
var HASH_STYLE_REGEX_META_ORDER = File.HASH_STYLE_REGEX_META_ORDER = /#([a-zA-Z-_]+?)[ \t]+(.+)$/gm;
var LINK_STYLE_REGEX = File.LINK_STYLE_REGEX = /(\[.*\]\s?)?(\[(.+)\]\(#([a-zA-Z-_]+?)(:)(-?[\d.]+(?:e-?\d+)?)?\))/gm;
var LINK_STYLE_REGEX_META_ORDER = File.LINK_STYLE_REGEX_META_ORDER = /(\[.*\]\s?)?(\[(.+)\]\(#([a-zA-Z-_]+?)\))/gm;
...
File.prototype.extractHashStyleTasksNoOrder = function(config, pos, content) {
  var hashStyleRegex = new RegExp(HASH_STYLE_REGEX_META_ORDER);
  var lang = this.getLang()
  var result;
  while((result = hashStyleRegex.exec(content)) !== null) {
...
File.prototype.deleteTask = function(task, config) {
  ...
  if (task.type === Task.Types.CODE) {
    ...
    this.deleteHashMetaOrderTask(HASH_STYLE_REGEX_META_ORDER, task, config);
...
File.prototype.modifyTask = function(task, config, noEmit) {
  if (task.type === Task.Types.CODE) {
    ...
    this.modifyHashMetaOrderTask(HASH_STYLE_REGEX_META_ORDER, task, config, noEmit);
  } else if (task.type === Task.Types.MARKDOWN) {
...
File.prototype.modifyHashMetaOrderTask = function(re, task, config, noEmit) {
  ...
  while ((result = re.exec(this.getContent())) !== null) {
    ...
    if (this.tasksMatch(task, line, taskText)) {
      ...
      if (noEmit !== 'fromContent') task.updateOrderMeta()
      this.modifyDescription(task, config)
      this.setModified(true);
      if (!noEmit) {
