...
function File(opts) {
  ...
  this.projectContext = appContext.get(FileProjectContext)
...
const HASH_STYLE_PATTERN = (File.HASH_STYLE_PATTERN =
  /#([a-zA-Z-_]+?):(-?[\d.]+(?:e-?\d+)?)?[ \t]+(.+)$/gm)
const HASH_STYLE_META_ORDER_PATTERN = (File.HASH_STYLE_META_ORDER_PATTERN =
  /#([a-zA-Z-_]+?)[ \t]+(.+)$/gm)
const LINK_STYLE_PATTERN = (File.LINK_STYLE_PATTERN =
  /(\[.*\]\s?)?(\[(.+)\]\(#([a-zA-Z-_]+?)(:)(-?[\d.]+(?:e-?\d+)?)?\))/gm)
...
File.removeFromContent = function (content, toRemove) {
...
File.prototype.removeMetaData = function (content, key, value) {
  return File.removeFromContent(
    content,
    `${key}${this.projectContext.config.getMetaSep()}${value}`
  )
}
...
File.prototype.transformTasks = function (config, modify) {
  ...
  fastSort(this.getTasks())
    ...
    .forEach((task) => {
      ...
      if (content === task.content && !trackListChanges.beforeTextModified)
        return
...
File.TASK_TYPE_TEMPLATES = {
  HASHTAG: (t) =>
    `${t.beforeText || ''}#${t.list}${t.hasColon ? ':' : ''}${t.getOrder()} ${
      t.text
    }`,
  HASH_META_ORDER: (t) => `${t.beforeText || ''}#${t.list} ${t.text}`,
  MARKDOWN: (t) =>
    `${t.beforeText || ''}[${t.text}](#${t.list}:${t.getOrder()})`,
}
...
File.prototype.modifyTask = function (task, config, noEmit) {
  if (task.type === Task.Types.CODE) {
    ...
    task.preserveBlankLines = hasBlankLines(description.join(eol.lf))
    if (task.preserveBlankLines) {
      description.unshift('<card>')
      description.push('</card>')
    }
    const text = File.TASK_TYPE_TEMPLATES[task.type](task)
  ...
  //   this.modifyHashMetaOrderTask(
  //     HASH_STYLE_META_ORDER_PATTERN,
  //     task,
  //     config,
  //     noEmit
  //   )
...
//       var beforeContent = this.getContent().substring(0, linePos)
//       var afterContent = this.getContent().substring(re.lastIndex)
//       var taskContent = `${task.beforeText}[${task.text}](#${
//         task.list
//       }:${task.getOrder()})`
