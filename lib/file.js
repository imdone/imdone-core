...
File.prototype.removeCompleted = function ({ task, content, config }) {
...
File.addStarted = function ({ task, content, config }) {
...
File.prototype.modifyTaskFromContent = function (task, content, config) {
  content = trimBlankLines(content, this.isCodeFile(), config)
  task.updateFromContent(content)
  this.modifyTask(task, config)
}

const DEFAULT_BLANK_LINE_TERMINATOR_COUNT = 2
const CODE_FILE_BLANK_LINE_PATTERN = /\n\s*\n/g
const CODE_FILE_BLANK_LINE_REPLACEMENT = '\n'

function getBlankLineTerminatorCountFromConfig(config) {
  const hasNumericTerminator = config
    && typeof config.customCardTerminator === 'number'
    && config.customCardTerminator > 0

  return hasNumericTerminator
    ? config.customCardTerminator
    : DEFAULT_BLANK_LINE_TERMINATOR_COUNT
}

function buildExcessiveBlankLinePattern(terminatorCount) {
  const minNewlinesToTrim = terminatorCount + 1
  const patternParts = Array(minNewlinesToTrim).fill('\\n\\s*')
  return new RegExp(patternParts.join(''), 'g')
}

function calculateMaxPreservedBlankLines(terminatorCount) {
  return terminatorCount - 1
}

function buildBlankLineReplacement(maxBlankLines) {
  const newlineCount = maxBlankLines + 1
  return '\n'.repeat(newlineCount)
}

const trimBlankLines = File.trimBlankLines = function (content, isCodeFile, config) {
  if (isCodeFile) {
    return content.replace(CODE_FILE_BLANK_LINE_PATTERN, CODE_FILE_BLANK_LINE_REPLACEMENT)
  }

  const terminatorCount = getBlankLineTerminatorCountFromConfig(config)
  const excessiveBlankLinePattern = buildExcessiveBlankLinePattern(terminatorCount)
  const maxBlankLines = calculateMaxPreservedBlankLines(terminatorCount)
  const replacement = buildBlankLineReplacement(maxBlankLines)

  return content.replace(excessiveBlankLinePattern, replacement)
}
...
File.prototype.getTaskAtLine = function (line) {
...
File.prototype.isWithinCodeSpanOrBlock = function(pos, content) {
  const positions = Task.getMarkdownCodePositions(content)
  return Task.isResultInMarkdownCode(positions, pos)
}

File.prototype.getMarkdownCodePositions = function (text) {
  const positions = [];
...
File.prototype.isResultInMarkdownCode = function (positions, index) {
