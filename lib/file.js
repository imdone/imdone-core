...
// Lets make sure TODO: we don't delete this whole line id:20
...
File.prototype.deleteCodeOrHashTask = function(re, task) {
  ...
  if (result) {
    ...
    if (this.tasksMatch(task, line, taskText)) {
      ...
      if (index < 0) index = 0;
      if (self.isCodeFile()) {
        var commentPrefix = this.getContent().substring(task.commentStart, index);
        var lang = self.getLang();
        var symbolRe = new RegExp(escapeRegExp(lang.symbol) + "\\s*");
        if (symbolRe.test(commentPrefix)) index = task.commentStart;
