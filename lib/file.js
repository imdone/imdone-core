...
File.prototype.transformTask = function ({config, modify, task}) {
  ...
  // TODO: This should also be used in modifyTask
  if (!config.keepEmptyPriority && config.orderMeta && !isNumber(task.order)) {
    const tempDescContent = eol.split(content)
    const text = tempDescContent.shift()
    task.updateOrderMeta(config, tempDescContent.join(lineEnd))
    content = [text, ...task.description].join(eol.auto)
  } 
  if (
    content.trim() !== task.content.trim() 
    || trackListChanges.beforeTextModified
    || (task.orderModified && config.orderMeta)
    || (task.orderModified && !config.keepEmptyPriority)
  ) {
...
File.prototype.newTask = function ({
  ...
  // if (task.orderModified) {
  //   console.log(`task { type: '${task.type}' text: '${task.text}', path: '${task.path}', line: ${task.line}}`)
  //   console.log(`type: '${task.type}' task.order:[${task.order}] order:[${order}]`)
  // }
