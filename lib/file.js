...
var CODE_STYLE_PATTERN = File.CODE_STYLE_PATTERN = "([A-Z]+[A-Z-_]{2,})((:)(\\d+?\\.?\\d*?)?)?\\s+(.*)$";
...
File.prototype.getLang = function() {
  // #ARCHIVE: Allow a project to extend languages by adding them to config.json gh-iss:5
...
File.prototype.getCodeCommentRegex = function() {
  // #TODO: Allow languages to have multiple block comment styles, like html gh-iss:13
  var lang = this.getLang();
...
File.prototype.extractCodeStyleTasks = function(config, pos, content) {
  ...
  while((result = codeStyleRegex.exec(content)) !== null) {
    ...
    if (charBeforeList !== "#") {
      var list = result[1];
      if (config && !config.includeList(list)) return; // #BACKLOG: Save all lists found so we can present them to the user
...
// #ARCHIVE: This is not working on some files (Created in windows)
...
// [Implement #listname style tasks in any text file](#ARCHIVE:)
// [Allow code style tasks to be included on list name](#ARCHIVE:)
/**
 * Description
 * @method extractTasks
 * @return ThisExpression
 */
// #ARCHIVE: Have a seperate extract and modify method for code files and only look in comments and block comments
