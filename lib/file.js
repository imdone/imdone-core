...
File.parseDueDate = function (config, text) {
  ...
  return text.replace(/(\sdue)\s.*?\./gi, (dueMatch, p1) => {
    ...
    try {
      const due = chrono.parseDate(dueMatch, new Date(), {forwardDate: true})
      if (!due) return dueMatch
      dueString = getIsoDateWithOffset(due)
...
File.parseRemindDate = function (config, text) {
  if (text.includes(`remind${config.getMetaSep()}`)) return text
  let remindString
  return text.replace(/(\sremind)\s.*?\./gi, (remindMatch, p1) => {
    if (remindString) return remindMatch
    try {
      const remind = chrono.parseDate(remindMatch, new Date(), {forwardDate: true})
      if (!remind) return remindMatch
      remindString = getIsoDateWithOffset(remind)
      return `${p1.toLowerCase()}${config.getMetaSep()}${remindString}`
    } catch (e) {
      console.log(`Unable to parse remind date for ${text}`)
      return remindMatch
    }
  })
}
...
File.prototype.extractCheckboxTasks = function (config, pos, content) {
  ...
  while ((result = checkStyleRegex.exec(content)) !== null) {
    ...
    task.rawTask = File.getRawTask(type, text, list, task.order)
    tasks.push(task)
  }
  if (tasks) {
    const hasTaskAtLine = (line) => {
...
File.getRawTask = function (type, text, list, order) {
  ...
  } else if (appContext().repo.config.orderMeta) {
    return `#${list} ${text}`
  }
...
File.TASK_TYPE_TEMPLATES = {
  HASHTAG: (t, config) => {
    const colon = t.hasColon || (isNumber(t.order) && !config.orderMeta) ? ':' : '';
    return `${t.beforeText || ''}#${t.list}${colon}${
      config.orderMeta ? '' : t.order
    } ${t.text}`;
  },
  MARKDOWN: (t, config) =>
