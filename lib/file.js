...
File.prototype.trimCodeCommentStart = function(text) {
...
File.prototype.trimCodeBlockStart = function(text) {
  if (this.isCodeFile() && this.getLang().block) {
...
File.prototype.trimCodeBlockIgnore = function(text) {
...
File.prototype.getCommentsAfterLine = function(ln, comment) {
  const commentLines = comment.splt(os.EOL);
  if (commentLines.length > 1) {
    return commentLines.map(line => {
      let regex = new Re
    })
  }
  const lines = this.getContent().split(os.EOL).slice(ln);
  lines.forEach(line => {

  })
// DOING: it recognizes a single line comment after a given line number in a file id:24 gh:115
// it recognies a start of a multiline comment after a given line number in a file
// it recognizes a continuation of a multiline comment after a given line number in a file
// it stops checking lines when it runs into a TODO comment id:25 gh:116
};
