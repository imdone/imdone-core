  ...
  while ((result = codeStyleRegex.exec(content)) !== null) {
    ...
    if (this.startsWithCommentOrSpace(posInContent) && charBeforeList !== '#') {
      ...
      const taskStartOnLine = inBlockComment
        ...
        : pos - linePos
      var task = self.extractTaskDescription({
        taskStartOnLine,
        rawTask,
        config,
        list,
...
File.prototype.extractHashStyleTasks = function (config, pos, content) {
  ...
  while ((result = hashStyleRegex.exec(content)) !== null) {
    ...
    var task = this.extractTaskDescription({
      config,
      list,
      text,
      order,
...
File.prototype.modifyDescription = function (task, config) {
  ...
  let { rawDescription, isWrappedWithCardTag, hasCardEndTag } =
...
File.prototype.extractTaskDescription = function ({
  taskStartOnLine,
  rawTask,
  config,
  list,
  ...
  let { rawDescription, isWrappedWithCardTag } = this.getRawDescription({
    config,
    content,
    inBlockComment,
    beforeText,
...
File.prototype.getRawDescription = function ({
  ...
  const { rawDescription, isWrappedWithCardTag, hasCardEndTag } =
    this.getTaskContent({ config, content, inBlockComment, beforeText })
  ...
  if (
    ...
    rawDescription.pop()
  return { rawDescription, isWrappedWithCardTag, hasCardEndTag }
}
