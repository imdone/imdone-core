...
File.prototype.getLinePos = function(lineNo) {
  ...
  while ((result = re.exec(content)) !== null) {
    if (line == lineNo) return result.index;
...
File.prototype.getLineNumber = function(pos) {
  var lines = this.getContent().split(/^/gm);
  var len = 0;
...
File.prototype.modifyCodeOrHashTask = function(re, task) {
  ...
  if (result) {
    ...
    if (task.id == getTaskId(line, taskText)) {
 ...
 * @return ThisExpression
 */
// #DONE:10 Have a seperate extract and modify method for code files and only look in comments and block comments
