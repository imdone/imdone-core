...
File.prototype.getDescriptionChars = function(inCodeBlock, rawDescription) {
  ...
  if (lang.block
      && lang.block.ignore
      && (trimmed.startsWith(lang.block.ignore)
          || inCodeBlock)) return lang.block.ignore
  return lang.symbol
}
...
File.prototype.newTask = function(config, list, text, order, line, type, hasColon, content, inCodeBlock) {
  var self = this;
...
File.prototype.extractCodeStyleTasks = function(config, pos, content) {
  ...
  while((result = codeStyleRegex.exec(content)) !== null) {
    ...
    if (this.startsWithCommentOrSpace(posInContent) && charBeforeList !== "#") {
      ...
      var task = self.newTask(config, list, text, order, line, Task.Types.CODE, hasColon, content.substring(result.index), inCodeBlock);
...
File.prototype.tasksMatch = function(task, line, taskText) {
  return (task.id == getTaskId(this.getPath(), line, taskText) ||
          (_.has(task, 'meta.id[0]') && (task.meta.id[0] === Task.getMetaData(taskText).id[0]))
        );
};
...
File.prototype.modifyTaskFromHtml = function(task, html) {
  ...
  let descContent = this.getContent().substring(descStart, descEnd);
  const beforeDescContent = this.getContent().substring(0, descStart);
  const afterDescContent = this.getContent().substring(descEnd);
  let i = 0;
  descContent = descContent.replace(re, () => {
    let char = checks[i] ? 'x' : ' ';
    i++;
    return `- [${char}]`;
  })
...
File.prototype.modifyDescription = function(task) {
  ...
  log(`description: ${task.description.join(os.EOL)}`)
  if (task.description.length === task.rawDescription.length) return
  const descStart = this.getLinePos(task.line+1);
  ...
  const description = task.description.map(desc => `${spaces}${task.descriptionStartsWith} ${desc}`).join(os.EOL)+os.EOL
  if (task.originalDescriptionLength === 0) {
    beforeDescContent += description
  } else {
    const rawDescription = task.rawDescription.join(os.EOL)+os.EOL
