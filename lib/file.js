...
var LINK_STYLE_REGEX = File.LINK_STYLE_REGEX = /(\[.*\]\s?)?(\[(.+)\]\(#([\w\-]+?)(:)(-?[\d.]+(?:e-?\d+)?)?\))/gm;
...
File.prototype.getRawDescription = function({config, content, inCodeBlock, beforeText}) {
  const { rawDescription, preserveBlankLines} = this.getTaskContent({config, content, inCodeBlock, beforeText})
  ...
  return { rawDescription, preserveBlankLines }
}
...
File.prototype.getTaskContent = function({config, content, inCodeBlock, beforeText}) {
  ...
  return { rawDescription: commentLines, preserveBlankLines};
}
...
File.prototype.modifyDescription = function(task, config) {
  ...
  let { rawDescription, preserveBlankLines } = this.getRawDescription({config, content: contentWithTask, inCodeBlock: task.inCodeBlock, beforeText: task.beforeText})
...
File.prototype.newTask = function({
    ...
    // TODO:0 ## add beforeText for code files
...
File.prototype.extractHashStyleTasks = function(config, pos, content) {
  ...
  while((result = hashStyleRegex.exec(content)) !== null) {
    ...
    if (self.hasTaskAtLine(line)) continue
    var list = result[1];
    // if (config.ignoreList(list)) continue
    var rawTask = result[0];
...
File.prototype.modifyTask = function(task, config, noEmit) {
  if (config.ignoreList(task.list)) this.transformTask({task, content: task.getContent(), config})
  if (task.type === Task.Types.CODE) {
    this.modifyCodeOrHashTask(new RegExp(CODE_STYLE_PATTERN, "gm"), task, config, noEmit);
  } else if (task.type === Task.Types.HASHTAG) {
    this.modifyCodeOrHashTask(HASH_STYLE_REGEX, task, config, noEmit);
  } else if (task.type === Task.Types.MARKDOWN) {
...
File.addCompleted = function({task, content, config}) {
  ...
  content = Task.addToLastMetaInContent(content, `completed:${now}`, config.settings.cards.metaNewLine)
  const lines = eol.split(content)
  return lines.join(lf)
}

File.prototype.updateMetaData = function (config) {
  if (!_get(config, 'settings.cards.trackChanges') || this.isCodeFile()) return
  const lists = config.lists
                    .filter(list => !list.filter)
                    .map(list => list.name)
  this.getTasks().forEach(task => {
    if (task.hasListChanged(lists)) {
      const list = task.list
      const now = (new Date()).toISOString()
      const taskContent = task.content
      const content = Task.addToLastMetaInContent(taskContent, `${list}:${now}`, config.settings.cards.metaNewLine)
...
// DOING:10 ## Refactor transformContent and updateMetaData
// +1.13.0
File.prototype.transformContent = function ({task, content, config}) {
  if (this.isCodeFile()) return content
...
File.prototype.transformTask = function ({task, content, config}) {
  content = this.transformContent({task, content, config})
  if (content === task.getContent()) return false
...
File.prototype.modifyTaskFromContent = function(task, content, config) {
  if (this.transformTask({task, content, config})) {
