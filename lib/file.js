...
File.prototype.getLang = function() {
  // #ARCHIVE:20 Allow a project to extend languages by adding them to config.json gh-iss:5 id:3
...
File.prototype.getCodeCommentRegex = function() {
  // #TODO:10 Allow languages to have multiple block comment styles, like html gh-iss:13 id:4
  var lang = this.getLang();
...
File.prototype.extractCodeStyleTasks = function(config, pos, content) {
  ...
  while((result = codeStyleRegex.exec(content)) !== null) {
    ...
    if (charBeforeList !== "#") {
      var list = result[1];
      if (config && !config.includeList(list)) return; // #BACKLOG:20 Save all lists found so we can present them to the user id:5
...
// #ARCHIVE:10 This is not working on some files (Created in windows) id:6
...
// #ARCHIVE:40 This is a problem id:7
...
// [Implement #listname style tasks in any text file id:8](#ARCHIVE:160)
// [Allow code style tasks to be included on list name id:9](#ARCHIVE:150)
/**
 * Description
 * @method extractTasks
 * @return ThisExpression
 */
// #ARCHIVE:60 Have a seperate extract and modify method for code files and only look in comments and block comments id:10
