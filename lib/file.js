...
File.prototype.getLineNumber = function(pos) {
  return Task.getLineNumber(this.content, pos);
};
...
const getDoneList = function(config) {
...
File.prototype.transformTasks = function (config) {
  if (this.isCodeFile()) return
  ...
  fastSort(this.getTasks()).by({desc: 'line'}).forEach(task => {
    ...
    if (content === task.content && !beforeTextModified) return
...
File.prototype.extractTasks = function(config, transformTasks) {
  this.tasks = [];
  ...
  if (transformTasks) this.transformTasks(config)
