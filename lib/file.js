...
File.addCompleted = function({task, content, config}) {
  ...
  if (!task || !config || !config.settings || !doneList || !this.isAddCompletedMeta(config)) return content
  ...
  content = task.addToLastCommentInContent(content, `completed${config.getMetaSep()}${now}`, File.isMetaNewLine(config))
...
File.prototype.trackListChanges = function ({task, content, config, modify}) {
  ...
  if (this.isMarkDownFile()) {
    ...
    if (_get(config, 'settings.cards.trackChanges') && task.hasListChanged(lists)) {
      ...
      const now = (new Date()).toISOString()
      content = task.addToLastCommentInContent(content, `${list}${config.getMetaSep()}${now}`, File.isMetaNewLine(config))
    ...
    if (doneList !== task.list && task.meta.completed && task.meta.completed.length > 0) {
      content = task.removeFromContent(content, `completed${config.getMetaSep()}${task.meta.completed[0]}`)
    }
  }
...
const getOrder = function(config, order) {
  return isNaN(order) ? ( config && config.keepEmptyPriority ? '' : 0 ) : parseFloat(order);
}
...
File.prototype.extractCodeStyleTasks = function(config, pos, content) {
  ...
  while((result = codeStyleRegex.exec(content)) !== null) {
    ...
    if (this.startsWithCommentOrSpace(posInContent) && charBeforeList !== "#") {
      ...
      var order = getOrder(config, result[4]);
...
File.prototype.extractCheckboxTasks = function(config, pos, content) {
  ...
  const hasTaskAtLine = (line) => {
    return this.hasTaskAtLine(line) || tasks.find(task => {
...
File.prototype.deleteHashMetaOrderTask = function(re, task, config) {
  ...
  if (result) {
    ...
    var [rawTask, list, text] = result;
    if (this.tasksMatch(config, task, line, text)) {
  ...
  while ((result = re.exec(this.getContent())) !== null) {
    ...
    if (this.tasksMatch(config, task, line, text)) {
      ...
      if (noEmit !== 'fromContent') {
        ...
        task.updateOrderMeta(config, newLine)
      }
      this.modifyDescription(task, config)
      this.setModified(true);
      if (!noEmit) {
  ...
  while ((result = re.exec(this.getContent())) !== null) {
    ...
    if (this.tasksMatch(config, task, line, taskText)) {
      ...
      if (task.inCodeBlock) {
        ...
        if (blockEnd > -1) {
          const desc = (task.description.length > 0) ? `${lineEnd}${task.description.join(lineEnd)}` : ''
      ...
      var taskContent = `${task.beforeText || ''}${hash}${task.list}${task.hasColon ? ":" : ""}${task.getOrder()} ${text}`
      task.line = this.getLineNumber(index)
      task.id = getTaskId(self.getPath(), task.line, task.text);
      this.setContent(beforeContent + taskContent + afterContent);
...
File.prototype.modifyLinkTask = function(task, config, noEmit) {
  ...
  while ((result = re.exec(content)) !== null) {
    ...
    if (this.tasksMatch(config, task, line, text)) {
      ...
      var beforeContent = this.getContent().substring(0, linePos);
      var afterContent = this.getContent().substring(re.lastIndex);
      var taskContent = `${task.beforeText}[${task.text}](#${task.list}:${task.getOrder()})`
...
File.prototype.modifyDescription = function(task, config) {
  ...
  if (descStart === this.getContent().length && !beforeDescContent.endsWith(lf)) beforeDescContent += lineEnd
...
File.prototype.tasksMatch = function(config, task, line, taskText) {
  return (task.id == getTaskId(this.getPath(), line, taskText) ||
          (task.meta.id && Task.getMetaData(config, taskText).id && (task.meta.id[0] === Task.getMetaData(config, taskText).id[0]))
        );
};
...
File.prototype.ignoreCodeBlocks = function() { // BACKLOG:-100 This is not used, but we need a way to ignore tasks in code blocks. Maybe save position ranges
...
File.prototype.getTaskContent = function({config, content, inCodeBlock, beforeText}) {
  ...
  for(let i = preserveBlankLines ? 2 : 1; i < lines.length; i++) {
    let line = lines[i];
    if (this.isCheckBoxTask(config, line, beforeText) && !preserveBlankLines) break
