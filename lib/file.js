...
var getTaskId = File.getTaskId = function(path, line, text, rawDescription) {
  var shasum = crypto.createHash('sha1');
  const taskElements = {path, line, text: text.trim(), rawDescription}
...
File.prototype.newTask = function({
    ...
    var task = new Task({
      ...
      order,
      line,
      id: getTaskId(self.getPath(), line, text, rawDescription),
...
File.prototype.tasksMatch = function(task, line, taskText, rawDescription) {
  return (task.id == getTaskId(this.getPath(), line, taskText, rawDescription) ||
...
File.prototype.deleteHashMetaOrderTask = function(re, task, config) {
  ...
  if (result) {
    ...
    var [rawTask, list, text] = result;
    if (this.tasksMatch(task, line, text, task.rawDescription)) {
...
File.prototype.deleteCodeOrHashTask = function(re, task, config) {
  ...
  if (result) {
    ...
    if (this.tasksMatch(task, line, taskText, task.rawDescription)) {
      var index = result.index;
      var afterStart = re.lastIndex;
      if (index < 0) index = 0;
      if (self.isCodeFile()) {
...
File.prototype.modifyHashMetaOrderTask = function(re, task, config, noEmit) {
  ...
  while ((result = re.exec(this.getContent())) !== null) {
    log('Got result: %j', result);
    let text = result[2]
    const {rawDescription} = this.getRawDescription({config, content: this.getContent(), inCodeBlock: this.isInCodeBlock(result[0]), beforeText: this.getBeforeText(line, result.index)})
    if (this.tasksMatch(task, line, text, rawDescription)) {
      ...
      task.id = getTaskId(self.getPath(), task.line, task.text, task.description);
...
File.prototype.modifyCodeOrHashTask = function(re, task, config, noEmit) {
  ...
  while ((result = re.exec(this.getContent())) !== null) {
    ...
    var list = result[1];
    var text = result[ task.type === Task.Types.HASHTAG ? 3 : 5 ];
    const {rawDescription} = this.getRawDescription({config, content: this.getContent(), inCodeBlock: this.isInCodeBlock(rawTask), beforeText: this.getBeforeText(line, result.index)})
    var taskText = this.trimCommentBlockEnd(text);
    if (this.tasksMatch(task, line, taskText, rawDescription)) {
      if (task.updatedText) task.text = task.updatedText 
...
File.prototype.modifyLinkTask = function(task, config, noEmit) {
  ...
  while ((result = re.exec(content)) !== null) {
    ...
    const {rawDescription} = this.getRawDescription({config, content: this.getContent(), inCodeBlock: this.isInCodeBlock(match), beforeText: this.getBeforeText(line, result.index)})
    if (this.tasksMatch(task, line, text, rawDescription)) {
