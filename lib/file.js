...
File.prototype.getCommentsAfterLine = function(ln, comment) {
  ...
  if (lines.length < 2) {
    ...
    startOn = 0
    lines = this.getContent().split(os.EOL).slice(ln);
  }
  for(let i=startOn; i < lines.length; i++) {
    const line = lines[i];
    if (singleLineComment && line.indexOf(this.getLang().symbol) < 0) break;
    if (this.hasTaskInText(line)) break;
    const commentLine = this.trimCommentChars(line).trim()
    if (commentLine) commentLines.push(commentLine)
  }
  return commentLines;
// DOING: it recognizes a single line comment after a given line number in a file id:26 gh:119
// - [x] A task
...
// - it stops checking lines when it runs into another task
};

File.prototype.hasTaskInText = function(text) {
  return new RegExp(CODE_STYLE_PATTERN).test(text)
