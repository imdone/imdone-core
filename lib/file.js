...
File.prototype.isSingleLineBlockComment = function (content) {
  return content.split(os.EOL).length > 0
}
...
File.prototype.modifyCodeOrHashTask = function(re, task) {
  ...
  while ((result = re.exec(this.getContent())) !== null) {
    ...
    if (this.tasksMatch(task, line, taskText)) {
      ...
      var taskContent = util.format("%s%s%s%s %s", hash, task.list, task.hasColon ? ":" : "", order, text.trim());
      ...
      if (!task.singleLineBlockComment) this.modifyDescription(task)
      this.setModified(true);
