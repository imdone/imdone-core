...
File.prototype.getCommentsAfterLine = function(config, comment) {
  ...
  const singleLineComment = lines[0].indexOf(this.getLang().symbol) > -1
  for(let i=1; i < lines.length; i++) {
    const line = lines[i];
    if (this.isCodeFile() && singleLineComment && (line.indexOf(this.getLang().symbol) < 0 || line.trim().indexOf(this.getLang().symbol) > 0)) break;
    if (this.isCodeFile() && !singleLineComment && this.getLang().block && line.indexOf(this.getLang().block.end) > -1) break;
...
File.prototype.modifyDescription = function(task, config) {
  ...
  const description = task.description.map(desc => `${spaces}${task.descriptionStartsWith} ${desc}`).join(os.EOL)
