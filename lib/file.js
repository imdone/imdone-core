...
File.prototype.getTasksInCode = function(content, id, config) {
  ...
  // DONE:0 Now we need to look for multi-line comments
  if (this.getLang().block) {
    ...
    content.replace(regex, function(match, pos) {
      match.replace(new RegExp(blockPattern, "gm"), function(match, start, list, order, text, posInBlock) {
...
// DONE:10 Implement #listname style tasks in any text file
...
File.prototype.extractTasks = function(config) {
  ...
  this.tasks = [];
  // TODO:20 Store task type in task , so we know how to remove the task later
...
File.prototype.modifyTask = function(task, config) {
  ...
  if (this.getLang().block) {
    ...
    var regex = new RegExp(start + "[\\s\\S]*?" + end, "gm");
    var blockPattern = "^([\\s|" + start + "|" + ignore + "]*?" + CODE_STYLE_PATTERN;
    this.content = this.content.replace(regex, function(match) {
