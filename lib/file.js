...
// DOING: Rethink where we put this.  We might want it on the file object
File.parseDueDate = function(text) {
  if (text.indexOf('due:') > -1) return text
  let dueString
  return text.replace(/\sdue\s.*?\./ig, dueMatch => {
...
File.prototype.extractTasks = function(config) {
  ...
  this.tasks.forEach(task => {
    const updatedText = File.parseDueDate(task.text)
    if (task.text !== updatedText) {
      task.updatedText = updatedText
      this.modifyTask(task, config)
    }
  })
  return this;
