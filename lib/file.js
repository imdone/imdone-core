...
File.prototype.getCommentsAfterLine = function(config, comment) {
  ...
  for(let i=1; i < lines.length; i++) {
    const line = lines[i];
    if (this.isCodeFile() && singleLineComment && (line.indexOf(lang.symbol) < 0 || line.trim().indexOf(lang.symbol) > 0) || line.trim() === lang.symbol) break;
...
// DONE:0 This is adding spaces where they shouldn't be. gh:199 id:61
// - [x] Compare each line of the description after trimming spaces to determine if it should be replaced
