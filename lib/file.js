...
File.prototype.getCommentsAfterLine = function(config, comment) {
  ...
  for(let i=1; i < lines.length; i++) {
    ...
    if (this.isCodeFile() && !singleLineComment && lang.block && line.trim() === lang.block.end) break;
    ...
    if (line && lang.block) commentLines.push(line.replace(lang.block.end,''))
  }
  return commentLines;
}

// DOING:30 This is adding spaces where they shouldn't be. gh:199 id:61
...
File.prototype.modifyDescription = function(task, config) {
  ...
  let spaces = ''
  if (rawDescription.length > 0 && (rawDescription[0].indexOf(this.getLang().symbol) > -1 || task.inCodeBlock)) {
...
File.prototype.newTask = function({config, list, text, order, line, type, hasColon, content, inCodeBlock, singleLineBlockComment}) {
  ...
  if (!singleLineBlockComment) {
    rawDescription = this.getRawDescription(config, content)
...
// DOING:20 There has to be an easier way yto find Line Pos id:72 gh:212
// - count characters
// - split 
...
File.prototype.deleteBlankLine = function(lineNo) {
  ...
  var startOfNextLine = this.getLinePos(lineNo+2);
...
File.prototype.modifyCodeOrHashTask = function(re, task, config) {
  ...
  while ((result = re.exec(this.getContent())) !== null) {
    ...
    if (this.tasksMatch(task, line, taskText)) {
      ...
      // DOING:0 Fix issue with single line block comments id:41 gh:168!
      // - [ ] another line
      // - [ ] and another
