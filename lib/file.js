...
var LINK_STYLE_REGEX = File.LINK_STYLE_REGEX = /(\[.*\]\s?)?(\[(.+)\]\(#([a-zA-Z-_]+?)(:)(-?[\d.]+(?:e-?\d+)?)?\))/gm;
...
File.prototype.modifyDescription = function(task, config) {
  ...
  let { rawDescription, preserveBlankLines, hasCardEndTag } = this.getRawDescription({config, content: contentWithTask, inCodeBlock: task.inCodeBlock, beforeText: task.beforeText})
