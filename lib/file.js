...
File.prototype.trackListChanges = function ({ task, content, config, modify }) {
  ...
  if (this.isMarkDownFile()) {
    ...
    if (
      ...
      content = task.removeFromContent(
        content,
        `completed${config.getMetaSep()}${task.meta.completed[0]}`
      )
    }
  }
...
File.prototype.transformTasks = function (config, modify) {
  ...
  fastSort(this.getTasks())
    ...
    .forEach((task) => {
      ...
      if (
        !config.keepEmptyPriority &&
        config.isAddNewCardsToTop &&
        task.order == 0
      ) {
        task.order = -new Date().getTime()
      }
...
const getOrder = function (config, order) {
  return isNaN(order)
    ? config && config.keepEmptyPriority
      ? ''
      : 0
    : parseFloat(order)
}
