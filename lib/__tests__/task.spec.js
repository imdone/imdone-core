...
describe("Task static methods", () => {
  it("should find markdown code positions with state machine", () => {
    const text = "Text ```code``` and \`inline\`";
    const positions = Task.getMarkdownCodePositions(text, false);
    expect(positions.length).toBe(2);
  });

  it("should find markdown code positions with regex fallback", () => {
    const text = "Text ```code``` and \`inline\`";
    const positions = Task.getMarkdownCodePositions(text, true);
    expect(positions.length).toBe(2);
  });

  it("should find HTML comment positions with state machine", () => {
    const text = "Text <!-- comment1 --> more <!-- comment2 -->";
    const positions = Task.getCommentPositions(text, false);
    expect(positions.length).toBe(2);
  });

  it("should find HTML comment positions with regex fallback", () => {
    const text = "Text <!-- comment1 --> more <!-- comment2 -->";
    const positions = Task.getCommentPositions(text, true);
    expect(positions.length).toBe(2);
  });
});
