...
Repository.prototype.moveTask = function ({ task, newList, newPos }, cb) {
  if (!Task.isTask(task)) task = newCard(task, this.project, true)
  ...
  if (tasksAfter && !tasksBefore) {
    task.order = (Number.isNaN(taskAfter.order) ? -10 : taskAfter.order) - 10
    tasksToModify.push(task)
  } else if (tasksBefore && !tasksAfter) {
    task.order = (Number.isNaN(taskBefore.order) ? 10 : taskBefore.order) + 10
    tasksToModify.push(task)
  } else if (tasksBefore && tasksAfter) {
    const taskBeforeOrder = Number.isNaN(taskBefore.order)
      ? taskBefore.order
      : 0
    const taskAfterOrder = Number.isNaN(taskAfter.order) ? taskAfter.order : 0
    ...
    if (taskBeforeOrder === taskAfterOrder) {
      ...
      for (
        ...
        Number.isNaN(closestIndexWithDifferentOrder) && index > -1;
        index--
      ) {
      ...
      if (!Number.isNaN(closestIndexWithDifferentOrder)) {
