...
import { CodeStyleTaskParser } from './adapters/parsers/task/CodeStyleTaskParser.js'
...
const STREAM_FALLBACK_REASONS = {
  BLOCK_COMMENT_WRITE_BACK_CONTEXT_UNSUPPORTED: 'block-comment-write-back-context-unsupported',
}

async function readStreamTasks(filePath, config) {
  const parser = await getFileParser(filePath, config)
  const tasks = []
  let task
  try {
    while ((task = await parser.readTask())) {
      tasks.push(task)
    }
  } finally {
    parser.close()
...
function parseCodeStreamTask(streamTask, config) {
  if (!streamTask.content) return
  const firstTaskLine = streamTask.content
    .split(/\r?\n/)
    .find((line) => line.trim())
  if (!firstTaskLine) return

  const parser = new CodeStyleTaskParser({
    ...config,
    includeList: (list) => config.includeList(list),
    listExists: (list) => config.includeList(list),
  })
  const parsed = parser.parseLine(firstTaskLine, streamTask.line)
  if (!parsed) return

  parsed.lastLine = streamTask.lastLine
  return parsed
}

function toFileTask({ file, streamTask, config }) {
  const parsedTask = streamTask.text
    ? streamTask
    : parseCodeStreamTask(streamTask, config)
  if (!parsedTask) return

  const lineContent = file.getContentLines()[parsedTask.line - 1] || ''
  const type = parsedTask.type || Task.Types.CODE
  const taskStartOnLine = getTaskStartOnLine({ lineContent, parsedTask, type, config })
  const linePos = file.getLinePos(parsedTask.line)
  const tokenPrefix = type === Task.Types.HASHTAG ? config.tokenPrefix : ''
  const rawTask = parsedTask.rawTask || getOriginalRawTask({
    lineContent,
    parsedTask,
    taskStartOnLine,
    type,
  ...
  return file.extractTaskWithDescription({
    taskStartOnLine,
    rawTask,
    config,
    ...
    type,
    hasColon: !!parsedTask.colon,
    content: file.getContent().substring(linePos),
    inBlockComment: parsedTask.inBlockComment,
    singleLineBlockComment: parsedTask.singleLineBlockComment,
    pos: linePos + taskStartOnLine,
  })
}

function getTaskStartOnLine({ lineContent, parsedTask, type, config }) {
  if (parsedTask.beforeText) return parsedTask.beforeText.length
  if (type === Task.Types.CODE) {
    return Math.max(0, lineContent.indexOf(parsedTask.list))
  }
  if (type === Task.Types.HASHTAG) {
    const taskTokenIndex = lineContent.indexOf(`${config.tokenPrefix}${parsedTask.list}`)
    if (taskTokenIndex > -1) return taskTokenIndex
  }
  return 0
}

function getOriginalRawTask({
  lineContent,
  parsedTask,
  taskStartOnLine,
  type,
  ...
  if (type !== Task.Types.CODE) {
    return lineContent.slice(taskStartOnLine)
  }

  return getRawTask({
    tokenPrefix,
    orderMeta: config.orderMeta,
    beforeText: parsedTask.beforeText,
    hasColon: !!parsedTask.colon,
    list: parsedTask.list,
    order: parsedTask.order,
    text: parsedTask.text,
...
function attachStreamTasks({ file, streamTasks, config }) {
  file.tasks = []
  streamTasks.forEach((streamTask) => {
    const task = toFileTask({ file, streamTask, config })
    if (!task) return
    file.addTask(task)
    file.emit('task.found', task)
  })
}

function getStreamWriteBackFallbackReason(streamTasks) {
  if (streamTasks.some((streamTask) => streamTask.inBlockComment)) {
    return STREAM_FALLBACK_REASONS.BLOCK_COMMENT_WRITE_BACK_CONTEXT_UNSUPPORTED
  }
}

function emitParserDiagnostic(repo, file, diagnostic) {
  repo.emit('file.parser', {
    path: file.path,
    ...diagnostic,
  })
}
...
export class Repository extends Emitter {
  ...
  async extractTasks (file) {
    // INSTRUMENTATION: Track timing for hang investigation
    const extractStartTime = Date.now()
    logger.debug(`[INSTRUMENT] extractTasks START for ${file.path}`)
    ...
    logger.debug(`[INSTRUMENT] readFileContent took ${readTime - extractStartTime}ms`)

    file.on('task.found', this.taskFoundListener)
    file.on('task.modified', this.taskModifiedListener)

    const streamTasks = await readStreamTasks(this.getFullPath(file), this.getConfig())
    const fallbackReason = getStreamWriteBackFallbackReason(streamTasks)
    if (fallbackReason) {
      file.extractAndTransformTasks(this.getConfig())
      emitParserDiagnostic(this, file, {
        mode: 'legacy-fallback',
        reason: fallbackReason,
      })

      this.resetFile(file)
      const fallbackTotalTime = Date.now() - extractStartTime
      logger.debug(`[INSTRUMENT] extractTasks END (legacy-fallback) - total ${fallbackTotalTime}ms`)
      return file
    }

    const fileContent = file.content
    attachStreamTasks({ file, streamTasks, config: this.getConfig() })
    file.transformTasks(this.getConfig())
    emitParserDiagnostic(this, file, {
      mode: 'stream',
    })

    if (file.isModified() && fileContent !== file.content) {
      logger.debug('[INSTRUMENT] About to writeFile (stream re-extract flow)')
      await this.writeFile(file)
      file.extractTasks(this.getConfig())
    }

    this.resetFile(file)
    const streamTotalTime = Date.now() - extractStartTime
    logger.debug(`[INSTRUMENT] extractTasks END (stream) - total ${streamTotalTime}ms`)
    return file
  }
 ...
 async deleteFile (path, cb) {
  ...
  async writeAndExtract (file, emit) {
    logger.debug(`[INSTRUMENT] writeAndExtract START - file: ${file.path}`)

    try {
      logger.debug(`[INSTRUMENT] writeAndExtract - about to call writeFile`)
      await this.writeFile(file, emit)
      logger.debug(`[INSTRUMENT] writeAndExtract - writeFile completed`)
    } catch (err) {
      new Error('Unable to write file:' + file.path, {cause: err})
    }

    try {
      logger.debug(`[INSTRUMENT] writeAndExtract - about to call extractTasks`)
      const extractStart = Date.now()
      await this.extractTasks(file)
      const extractTime = Date.now() - extractStart
      logger.debug(`[INSTRUMENT] writeAndExtract - extractTasks completed in ${extractTime}ms`)
    } catch (err) {
      new Error('Unable to extract tasks for file:' + file.path, { cause: err })
    }

    try {
      logger.debug(`[INSTRUMENT] writeAndExtract - about to call addFile`)
      await this.addFile(file)
      logger.debug(`[INSTRUMENT] writeAndExtract - addFile completed`)
    } catch (err) {
      new Error('Unable to add file after extracting tasks: ' + file.path, { cause: err })
    }

    logger.debug(`[INSTRUMENT] writeAndExtract END`)
    return file
  }
  ...
  async deleteTask (task, cb) {
    ...
    if (file.getContentForFile().trim() === '' && file.isMarkDownFile()) {
      ...
      logger.log('Deleting empty file:', file.path)
  ...
  async appendTask({file, content, list}) {
    logger.debug(`[INSTRUMENT] appendTask START - file: ${file.path}, contentLength: ${content?.length || 0}`)
    ...
    logger.debug(`[INSTRUMENT] appendTask - existing file contentLength: ${fileContent?.length || 0}`)
    ...
    const finalContent = `${fileContent}${appendContent}${cardTerminator}`;
    logger.debug(`[INSTRUMENT] appendTask - final contentLength: ${finalContent?.length || 0}`)
    logger.debug(`[INSTRUMENT] appendTask - about to call file.setContent`)
    file.setContent(finalContent)

    logger.debug(`[INSTRUMENT] appendTask - about to call writeAndExtract`)
    await this.writeAndExtract(file)
    logger.debug(`[INSTRUMENT] appendTask - writeAndExtract completed`)

    const task = file.getTasks().find(task => task.text === text && task.list === list)
    logger.debug(`[INSTRUMENT] appendTask END - task found: ${!!task}`)
    return { file, task }
  }
  ...
  async addTaskToFile (filePath, list, content) {
    logger.debug(`[INSTRUMENT] repo.addTaskToFile START - filePath: ${filePath}, list: ${list}\n${content}`)

    const relPath = this.getRelativePath(filePath)
    let file = this.getFile(relPath)

    if (file) {
      logger.debug(`[INSTRUMENT] repo.addTaskToFile - file exists, reading content`)
      await this.readFileContent(file)
      logger.debug(`[INSTRUMENT] repo.addTaskToFile - about to call appendTask`)
      return await this.appendTask({file, content, list})
    } else {
      logger.debug(`[INSTRUMENT] repo.addTaskToFile - creating new file`)
      ...
      logger.debug(`[INSTRUMENT] repo.addTaskToFile - about to call appendTask for new file`)
      return await this.appendTask({file, content, list})
    }
  }
  ...
  async modifyTask (task, writeFile = false) {
    ...
    try {
      ...
      logger.warn(`Can't get file for task: {text:'${task.text}', path:'${task.source.path}', line:${task.line}}`)
      throw e
    }
