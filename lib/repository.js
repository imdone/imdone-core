...
var errors                = { NOT_A_FILE: "file must be of type File",
    ...
    defaultFilePattern    = "^(readme\\.md|home\\.md)$",
    defaultExcludePattern = "^(node_modules|bower_components|\\.imdone|target|build)\\/?|\\.(git|svn)|\\~$|\\.(jpg|png|gif|swp|ttf|otf)$",
    configDir             = ".imdone",
    configFile            = configDir + "/config.json",
    defaultConfig         = { exclude: [defaultExcludePattern],
                              watcher: true,
                              marked : { 
                                gfm: true,
                                tables: true,
...
Repository.prototype.readFiles = function(files, cb) {
  ...
  // TODO:0 replace with async.parallel
  var self = this, error;
