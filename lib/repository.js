...
// DOING:0 Emit file.saved for individual files being saved then update checksum in writeFile id:1233
...
Repository.prototype.emitFileUpdate = function(file) {
  ...
  if (this.lastMovedFiles) {
    ...
    if (index > -1) {
      ...
      if (file && file.updated) this.emit("file.update", file);
    }
  } else {
    if (file && file.updated) this.emit("file.update", file);
  }
...
// [Make sure lists are not saved on load in alphabetical order id:1234](#ARCHIVE:170)
/**
 ...
 * @return CallExpression
 */
// #ARCHIVE:80 Make all calls to getFilesInPath async id:1235
...
// [Test renameList id:1236](#ARCHIVE:280)
/**
 * Description
 * @method renameList
 * @param {} oldName
...
Repository.prototype.renameList = function(oldName, newName, cb) {
  ...
  // DOING:20 Let's use saveModifiedFiles here id:1237
  if (funcs.length > 0) {
...
// #ARCHIVE:100 modifyTask must be async.  Get file content before proceeding. id:1238
/**
 * Description
 * @method modifyTask
 * @param {} task
...
// #ARCHIVE:120 Implement moveTasks id:1239
Repository.prototype.moveTasks = function(tasks, newList, newPos, cb) {
  ...
  var moveTask = function(task, newList, newPos, cb) {
    ...
    var taskMapper = function(_task, index) {
          return function(cb) {
            if (self.config.useImdoneioForPriority) {
              _task.io_order = index;
            } else if ( _task.order !== "" ) {
    ...
    async.series(_.map(toListTasks, taskMapper), function(err, results) {
      ...
      if (!sameList) {
        ...
        async.series(_.map(fromListTasks, taskMapper), cb);
      } else cb(null, task);
    });
  };
  ...
  async.series(_.map(tasks, function(task, i) {
    ...
    self.saveModifiedFiles(function(err, results) {
      if (!err) self.emit('tasks.moved', tasks);
