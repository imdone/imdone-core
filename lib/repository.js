...
Repository.prototype.addList = function(list, cb) {
  ...
  var fn = function(err) {
    cb(err);
    if (!err) self.emit('list.modified', list.name);
  };
 ...
 * @return CallExpression
 */
// DONE:0 Make all calls to getFilesInPath async
...
// DONE:0 modifyTask must be async.  Get file content before proceeding. 
/**
 * Description
 * @method modifyTask
 * @param {} task
...
Repository.prototype.modifyTask = function(task, cb) {
  ...
  } else cb(null, file.modifyTask(task, self.getConfig()));    
};


// DONE:10 Implement moveTasks
Repository.prototype.moveTasks = function(tasks, newList, newPos, cb) {
  ...
  var moveTask = function(task, newList, newPos, cb) {
    ...
    // console.log('toListTasks:%s', JSON.stringify(toListTasks, null, 3));
    // console.log('fromListTasks:%s', JSON.stringify(fromListTasks, null, 3));
  ...
  async.series(_.map(tasks, function(task, i) {
    ...
    var funcs = _.map(filesToSave, function(file) {
      return function(cb) { self.writeFile(file, cb); };
    });
...
Repository.prototype.getTasksInList = function(name) {
  ...
  return (tasks.length > 0) ? _.sortBy(_.where(this.getTasks(), {list:name}), ['order', 'text']) : [];
};
