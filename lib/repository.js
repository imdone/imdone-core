...
export class Repository extends Emitter {
 ...
 async deleteFile (path, cb) {
  ...
  async writeAndExtract (file, emit) {
    logger.debug(`[TRACE] writeAndExtract START - file: ${file.path}`)

    try {
      logger.debug(`[TRACE] writeAndExtract - about to call writeFile`)
      await this.writeFile(file, emit)
      logger.debug(`[TRACE] writeAndExtract - writeFile completed`)
    } catch (err) {
      new Error('Unable to write file:' + file.path, {cause: err})
    }

    try {
      logger.debug(`[TRACE] writeAndExtract - about to call extractTasks`)
      const extractStart = Date.now()
      await this.extractTasks(file)
      const extractTime = Date.now() - extractStart
      logger.debug(`[TRACE] writeAndExtract - extractTasks completed in ${extractTime}ms`)
    } catch (err) {
      new Error('Unable to extract tasks for file:' + file.path, { cause: err })
    }

    try {
      logger.debug(`[TRACE] writeAndExtract - about to call addFile`)
      await this.addFile(file)
      logger.debug(`[TRACE] writeAndExtract - addFile completed`)
    } catch (err) {
      new Error('Unable to add file after extracting tasks: ' + file.path, { cause: err })
    }

    logger.debug(`[TRACE] writeAndExtract END`)
    return file
  }
  ...
  async appendTask({file, content, list}) {
    logger.debug(`[TRACE] appendTask START - file: ${file.path}, contentLength: ${content?.length || 0}`)
    ...
    logger.debug(`[TRACE] appendTask - existing file contentLength: ${fileContent?.length || 0}`)
    ...
    logger.debug(`[TRACE] appendTask - final contentLength: ${finalContent?.length || 0}`)
    logger.debug(`[TRACE] appendTask - about to call file.setContent`)
    file.setContent(finalContent)

    logger.debug(`[TRACE] appendTask - about to call writeAndExtract`)
    await this.writeAndExtract(file)
    logger.debug(`[TRACE] appendTask - writeAndExtract completed`)

    const task = file.getTasks().find(task => task.text === text && task.list === list)
    logger.debug(`[TRACE] appendTask END - task found: ${!!task}`)
    return { file, task }
  }
  ...
  async addTaskToFile (filePath, list, content) {
    logger.debug(`[TRACE] repo.addTaskToFile START - filePath: ${filePath}, contentLength: ${content?.length || 0}`)

    const relPath = this.getRelativePath(filePath)
    let file = this.getFile(relPath)

    if (file) {
      logger.debug(`[TRACE] repo.addTaskToFile - file exists, reading content`)
      await this.readFileContent(file)
      logger.debug(`[TRACE] repo.addTaskToFile - about to call appendTask`)
      return await this.appendTask({file, content, list})
    } else {
      logger.debug(`[TRACE] repo.addTaskToFile - creating new file`)
      ...
      logger.debug(`[TRACE] repo.addTaskToFile - about to call appendTask for new file`)
      return await this.appendTask({file, content, list})
    }
  }
