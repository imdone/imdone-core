...
Repository.prototype.shouldInclude = function (relPath) {
  ...
  let include = true
  try {
    if (this.ignore && relPath) {
      include = !this.ignore.ignores(relPath)
    }
  } catch (e) {
    console.error(
...
Repository.prototype.readFile = function (file, cb) {
  ...
  if (!/\.\.(\/|\\)/.test(file.path)) {
    this.readFileContent(file, (err, file) => {
      ...
      // BACKLOG:-40 Allow user to assign a module for content transformation
...
Repository.prototype.renameList = function (oldName, newName, cb) {
  ...
  if (listToChange.filter) {
    ...
    const filteredListToChange = filteredLists.find(
  ...
  // BACKLOG:-730 Let's use saveModifiedFiles here id:15 gh:100 +fix
...
Repository.prototype.deleteTask = function (task, cb) {
  ...
  const execute = function (file) {
    ...
    if (
      file.getContentForFile().trim() === '' &&
      self.config.journalType === constants.JOURNAL_TYPE.NEW_FILE
    ) {
      return self.deleteFile(file.path, cb)
    }
...
Repository.prototype.appendTask = function({file, content, list}, cb) {
  ...
  const description =
    lines.length > 1 ? Task.padDescription(lines.slice(1), taskPrefix) : []
  ...
  if (taskSyntax === Task.Types.MARKDOWN) {
    appendContent = `${taskPrefix}[${text}](#${list}:${
      config.orderMeta ? '' : order
    })${eol.lf}${taskContent}`
  } else if (taskSyntax === Task.Types.HASH_META_ORDER) {
    appendContent = `${taskPrefix}#${list} ${text}${eol.lf}${taskContent}`
  } else {
    appendContent = `${taskPrefix}#${list}:${
      config.orderMeta ? '' : order
    } ${text}${eol.lf}${taskContent}`
  }
...
function getPreviousIndexWithDifferentOrder(tasks, pos) {
  ...
  for (
    ...
    const t = tasks[index]
    if (Task.isNumber(parseInt(t.order)) && !tasksHaveSameOrder(lastTask, t)) closestIndexWithOrder = index
    lastTask = t
  }
  return closestIndexWithOrder
}

function getNextIndexWithOrder(tasks, pos) {
  let closestIndexWithOrder = -1

  let lastTask = tasks[pos-1]
  ...
  for (
    ...
    const t = tasks[index]
    if (!Task.isNumber(t.order) || (Task.isNumber(t.order) && !tasksHaveSameOrder(lastTask, t))) closestIndexWithOrder = index
    lastTask = t
  }
  return closestIndexWithOrder
}

function tasksHaveOrder(taskA, taskB) {
  return taskA && taskB && Task.isNumber(taskA.order) && Task.isNumber(taskB.order)
}
...
Repository.prototype.moveTask = function ({ task, newList = task.list, newPos }, cb) {
  ...
  const taskBefore = toListTasks[newPos - 1]
  
  function getOrderFromTasksBefore() {
    const previousIndexWithDifferentOrder = getPreviousIndexWithDifferentOrder(toListTasks, newPos)
    let newOrder = previousIndexWithDifferentOrder > -1 ? toListTasks[previousIndexWithDifferentOrder].order + 10 : 0
    for (let index = previousIndexWithDifferentOrder + 1; index <= newPos; index++) {
  ...
  function getOrderFromTasksAfter(nextIndexWithOrder) {
    const startingOrder = taskBefore.order
    const endingOrder = toListTasks[nextIndexWithOrder].order
    const changes = nextIndexWithOrder - newPos
    const increment = isNumber(endingOrder) ? (endingOrder - startingOrder) / changes : 10
    let newOrder = startingOrder
    for (let index = newPos; index < nextIndexWithOrder; index++) {
      newOrder += increment
      toListTasks[index].order = newOrder
      tasksToModify.push(toListTasks[index])
    }
  ...
  const nextIndexWithOrder = getNextIndexWithOrder(toListTasks, newPos)
  const taskAfter = toListTasks[newPos + 1]
  const bothHaveOrder = tasksHaveOrder(taskBefore, taskAfter)
  const bothHaveSameOrder = tasksHaveSameOrder(taskBefore, taskAfter)
  if (bothHaveOrder && !bothHaveSameOrder) {
    task.order = taskBefore.order + (taskAfter.order - taskBefore.order)/2
    tasksToModify.push(task)
  } else if (bothHaveOrder && bothHaveSameOrder) {
    getOrderFromTasksAfter(nextIndexWithOrder)
  } else {
    getOrderFromTasksBefore()
  }
...
Repository.prototype.moveTasks = function (tasks, newList, newPos, noEmit, cb) {
  ...
  fastSort(tasks).by({asc: t => t.line})
  async.series(
    tasks.map((task, i) => {
      return function (cb) {
...
Repository.prototype.getTasksInList = function (name, offset, limit) {
  ...
  if (Task.isNumber(offset) && Task.isNumber(limit))
...
Repository.regexQuery = function (tasks, queryString) {
  return tasks.filter((task) => {
    if (
      task.rawTask.search(new RegExp(tools.escapeRegExp(queryString), 'i')) > -1
    )
      return true
    for (let i in task.description) {
      if (
        task.description[i].search(
          new RegExp(tools.escapeRegExp(queryString), 'i')
        ) > -1
      )
        return true
    }
