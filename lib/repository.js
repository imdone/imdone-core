...
// DOING: Emit file.saved for individual files being saved then update checksum in writeFile
...
Repository.prototype.addList = function(list, cb) {
  ...
  if (/^[^a-z]*$/.test(list.name) && this.config.code.include_lists.indexOf(list.name) < 0)
    this.config.code.include_lists.push(list.name);
  this.saveConfig(fn);
};
...
// [Make sure lists are not saved on load in alphabetical order](#ARCHIVE:)
/**
...
Repository.prototype.shouldExclude = function(relPath) {
  var exclude = false;
  if (this.ignore) {
    exclude = this.ignore.ignores(relPath);
  }
 ...
 * @return CallExpression
 */
// #ARCHIVE: Make all calls to getFilesInPath async
...
Repository.prototype.renameList = function(oldName, newName, cb) {
  ...
  // DOING: Let's use saveModifiedFiles here
  if (funcs.length > 0) {
...
// #ARCHIVE: modifyTask must be async.  Get file content before proceeding.
/**
 * Description
 * @method modifyTask
 * @param {} task
