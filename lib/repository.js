...
// DOING: Emit file.saved for individual files being saved then update checksum in writeFile id:11
...
Repository.prototype.createListeners = function() {
  ...
  this.taskListener = function(event, task) {
    if (!self.listExists(task.list)) {
      ...
      if (self.config.includeList(list.name)) self.addList(list); // DONE: check if this list is in include_lists before adding it id:16 +enhancement gh:79
...
Repository.prototype.readFiles = function(files, cb) {
  ...
  var exec = function(files) {
    ...
    if (funcs.length > 0) async.parallelLimit(funcs, 1024, cb);
    else cb(null, []);
  };
