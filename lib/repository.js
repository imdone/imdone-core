...
Repository.prototype.moveTask = function ({ task, newList = task.list, newPos }, cb) {
  ...
  function getTasksToModify(task, toListTasks, newPos) {
    if (toListTasks.length === 0) return 0

    const previousIndexWithDifferentOrder = getPreviousIndexWithDifferentOrder(toListTasks, newPos)
    const nextIndexWithDifferentOrder = getNextIndexWithDifferentOrder(toListTasks, newPos)
    const previousTaskWithOrder = toListTasks[previousIndexWithDifferentOrder]
    const nextTaskWithOrder = toListTasks[nextIndexWithDifferentOrder]
    ...
    const end = nextIndexWithDifferentOrder > -1 ? nextIndexWithDifferentOrder : toListTasks.length - 1;
    const changes = nextIndexWithDifferentOrder - previousIndexWithDifferentOrder
