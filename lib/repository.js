...
Repository.prototype.updateMetaData = function (file) {
  file.updateMetaData(this.getConfig())
}
...
Repository.prototype.writeAndExtract = function (file, emit, cb) {
  this.writeFile(file, emit, (err, file) => {
    ...
    this.extractTasks(file, (err, file) => {
      ...
      this.addFile(file, function(err) {
...
Repository.prototype.addTaskToFile = function(filePath, list, content, order, cb) {
  ...
  const interpretedTaskPrefix = _template(_get(config, 'settings.taskPrefix', ''))({date: new Date()})
  const taskPrefix = interpretedTaskPrefix.trim() === '' ? '' : `${interpretedTaskPrefix} `
  ...
  const appendTask = (file, cb) => {
    ...
    let appendContent 
    if (taskSyntax === "MARKDOWN") {
      appendContent = `${pre}${taskPrefix}[${text}](#${list}:${order})${eol.lf}${description}` 
    } else if (taskSyntax === "HASH_META_ORDER") {
      const task = {description: lines, meta: {}, order}
      Task.prototype.updateOrderMeta.apply(task, [newLine])
      const taskContent = task.description.join(eol.lf)
      appendContent = `${pre}${taskPrefix}#${list} ${taskContent}`
    } else {
...
Repository.prototype.modifyTask = function(task, writeFile, cb) {
  ...
  if (!file.getContent()) {
    this.readFileContent(file, (err, file) => {
      ...
      if (writeFile) return this.writeAndExtract(file, true, cb);
      cb(null, task);
    });
  } else {
    file.modifyTask(task, self.getConfig());
...
Repository.prototype.moveTask = function({task, newList, newPos, noEmit}, cb) {
...
Repository.prototype.runFilteredListQuery = function(list, tasks) {
  try {
    list.tasks = Repository.query(tasks, list.filter).map(task => {
      const filteredListTask = new Task({...task, filteredListName: list.name})
