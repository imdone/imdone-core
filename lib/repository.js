...
export class Repository extends Emitter {
  ...
  async extractTasks (file) {
    ...
    logger.debug(`[INSTRUMENT] About to call extractAndTransformTasks, content length: ${fileContent?.length || 0}`)
    file.extractAndTransformTasks(this.getConfig())

    const transformTime = Date.now()
    logger.debug(`[INSTRUMENT] extractAndTransformTasks took ${transformTime - readTime}ms`)

    if (!file.isModified() || fileContent === file.content) {
      this.resetFile(file)
      const totalTime = Date.now() - extractStartTime
      logger.debug(`[INSTRUMENT] extractTasks END (no changes) - total ${totalTime}ms`)
      return file
    }

    logger.debug(`[INSTRUMENT] About to call file.extractTasks`)
    file.extractTasks(this.getConfig())

    const extractTime = Date.now()
    logger.debug(`[INSTRUMENT] file.extractTasks took ${extractTime - transformTime}ms`)

    if (file.modified) {
      logger.debug(`[INSTRUMENT] About to writeFile`)
      await this.writeFile(file)
      logger.debug(`[INSTRUMENT] writeFile took ${Date.now() - extractTime}ms`)
    }
    ...
    const totalTime = Date.now() - extractStartTime
    if (totalTime > 1000) {
      logger.warn(`[INSTRUMENT] ⚠️  SLOW extractTasks: ${file.path} took ${totalTime}ms`)
    }
    logger.debug(`[INSTRUMENT] extractTasks END - total ${totalTime}ms`)
 ...
 async deleteFile (path, cb) {
  ...
  async addTaskToFile (filePath, list, content) {
    logger.debug(`[INSTRUMENT] repo.addTaskToFile START - filePath: ${filePath}, contentLength: ${content?.length || 0}`)
