...
Repository.prototype.addTaskToFile = function (filePath, list, content, cb) {
  ...
  const appendTask = (file, cb) => {
    ...
    const order = appContext.get(FileProjectContext).getOrder(list)
    const description =
  ...
  if (file) {
    ...
    const journalTemplate = config.journalTemplate
    const content =
      journalTemplate && journalTemplate.trim()
...
Repository.prototype.moveTask = function ({ task, newList, newPos }, cb) {
  ...
  if (tasksAfter && !tasksBefore) {
    task.order = (isNumber(taskAfter.order)
      ? taskAfter.order 
      : -10) - 10;
    tasksToModify.push(task)
  } else if (tasksBefore && !tasksAfter) {
    task.order = (isNumber(taskBefore.order) ? taskBefore.order : 10) + 10
    tasksToModify.push(task)
  } else if (tasksBefore && tasksAfter) {
    const taskBeforeOrder = isNumber(taskBefore.order)
      ? taskBefore.order
      : 0
    const taskAfterOrder = isNumber(taskAfter.order) ? taskAfter.order : 0
    ...
    if (taskBeforeOrder === taskAfterOrder) {
      ...
      for (
        let index = startOfList.length - 1;
        !isNumber(closestIndexWithDifferentOrder) && index > -1;
        index--
      ) {
      ...
      if (isNumber(closestIndexWithDifferentOrder)) {
        const closestTask = toListTasks[closestIndexWithDifferentOrder]
        const startingOrder = closestTask && isNumber(closestTask.order) ? closestTask.order : 0
        const numTasksWithSameOrder = newPos - closestIndexWithDifferentOrder
        let increment =
          (taskAfterOrder - startingOrder) / (numTasksWithSameOrder + 1)
        if (!isNumber(increment)) increment = 1
        let newOrder = startingOrder + increment
        for (
          let index = closestIndexWithDifferentOrder + 1;
          index < newPos + 1;
          index++
        ) {
      ...
      let increment = (taskAfterOrder - taskBeforeOrder) / 2
      task.order = taskBeforeOrder + increment
      tasksToModify.push(task)
    }
  } else {
    if (!isNumber(task.order)) task.order = 0
    tasksToModify.push(task)
  }

  async.eachSeries(
    fastSort(tasksToModify).by({ desc: 'line' }),
    (task, cb) => {
...
Repository.prototype.getTasksInList = function (name, offset, limit) {
  ...
  if (_isNumber(offset) && _isNumber(limit))
