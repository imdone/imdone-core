...
// DOING:0 ## Add project with defaultView and filterView
// Called from repo-fs-store mixin after files are parsed and cards extracted
// ### project
// Calls plugins
...
// - totals: all totals
// ### card
// Has fully parsed cards for display with all plugins utilized for getCardProperties([card](#card)
// <hr>
// expand:1
// epic:"release 1.3"
...
Repository.prototype.runFilteredListQuery = function(list, tasks) {
  try {
    list.tasks = Repository.query(tasks, list.filter).map(task => {
      const filteredListTask = newCard({...task, filteredListName: list.name}, this.project)
