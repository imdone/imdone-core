...
function getPreviousIndexWithOrder(tasks, pos) {
  let closestIndexWithOrder = -1

  for (
    let index = pos - 1;
    index > -1 && closestIndexWithOrder < 0;
    index--
  ) {
    const t = tasks[index]
    if (Task.isNumber(parseInt(t.order))) closestIndexWithOrder = index
  }
...
Repository.prototype.moveTask = function ({ task, newList = task.list, newPos }, cb) {
  ...
  function getOrderFromTasksBefore() {
    const closestIndexWithOrder = getPreviousIndexWithOrder(toListTasks, newPos)
    let newOrder = closestIndexWithOrder > -1 ? toListTasks[closestIndexWithOrder].order + 10 : 0
    for (let index = closestIndexWithOrder + 1; index < newPos + 1; index++) {
  ...
  function getOrderFromTasksAfter(nextIndexWithOrder) {
    const taskAfter = toListTasks[nextIndexWithOrder]
    task.order = taskBefore ? (taskAfter.order - taskBefore.order)/2 : taskAfter.order - 10
  ...
  if (taskBefore && taskAfter && Task.isNumber(taskBefore.order) && Task.isNumber(taskAfter.order)) {
    task.order = (taskAfter.order - taskBefore.order)/2
    tasksToModify.push(task)
  } else if (nextIndexWithOrder > -1) getOrderFromTasksAfter(nextIndexWithOrder)
  if (nextIndexWithOrder > -1) getOrderFromTasksAfter(nextIndexWithOrder)
  else getOrderFromTasksBefore()
