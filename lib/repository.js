...
Repository.prototype.createListeners = function() {
  ...
  this.taskFoundListener = function(task) {
    var index = _.findIndex(self.lists, {name:task.list});
    if (index < 0) self.lists.push(new List(task.list, false));
  };
};
...
Repository.prototype.getFilesInPath = function() {
  ...
  return _.map(paths, function(path) { return new File(path); });
};
