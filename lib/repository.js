...
// [Make sure lists are not saved on load in alphabetical order](#ARCHIVE:100)
/**
...
Repository.prototype.getTask = function(path, id) {
  var relPath = this.getRelativePath(path);
  var file = _.find(this.files, {path:relPath});
  if (file) {
    return file.getTask(id);
  }
};
 ...
 * @return CallExpression
 */
// #ARCHIVE:10 Make all calls to getFilesInPath async
    ...
    var funcs = _.map(files, function(file) {
      return function(cb) { 
        self.readFile(file, function(err, file) {
          completed++;
          self.emit('file.read', {path:file.path, percentComplete:(completed/files.length)});
          cb(err, file);
        }); 
      };
    });
...
// #ARCHIVE:30 modifyTask must be async.  Get file content before proceeding. 
/**
 * Description
 * @method modifyTask
 * @param {} task
...
// #ARCHIVE:50 Implement moveTasks
...
Repository.prototype.getTasksInList = function(name) {
  ...
  return (tasks.length > 0) ? _.sortBy(_.where(tasks, {list:name}), ['order', 'text']) : [];
};
...
Repository.deserialize = function(repo, cb) {
  ...
  _.each(repo.files, function(file) {
    log("file:%j", file);
    newRepo.addFile(new File(file), function(err) {
      if (err) return cb(err);
      count++;
