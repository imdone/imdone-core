...
Repository.getTasksByList = function (repo, tasksAry, noSort) {
  ...
  lists.forEach(function(list) {
    if (list.filter) return (list.tasks = repo.getFilteredTasks(list))
    list.tasks = tasks[list.name] || [];
  });
  return lists;
}

Repository.prototype.getFilteredTasks = function(list) {
  const filter = tools.replaceDateLanguage(list.filter)
  const tasks = Repository.query(this.getTasks(), filter)
  return tasks
};
