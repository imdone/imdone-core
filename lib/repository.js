...
Repository.prototype.addTaskToFile = function(filePath, list, content, order, cb) {
  ...
  const interpretedTaskPrefix = _template(_get(this.config, 'settings.taskPrefix', ''))({date: new Date()})
  ...
  const appendTask = (file, cb) => {
    ...
    if (taskSyntax === "MARKDOWN") {
      appendContent = `${pre}${taskPrefix}[${text}](#${list}:${order})\n${description}` 
    } else if (taskSyntax === "HASH_META_ORDER") {
      const task = {description: content.split(eol.lf), meta: {}, order}
      Task.prototype.updateOrderMeta.apply(task)
      content = task.description.join(eol.lf)
      appendContent = `${pre}${taskPrefix}#${list} ${content}`
    } else {
      appendContent = `${pre}${taskPrefix}#${list}:${order} ${content}`
    }
...
Repository.prototype.runFilteredListQuery = function(list, tasks) {
  try {
    list.tasks = Repository.query(tasks, list.filter).map(task => {
      const filteredListTask = new Task({...task, filteredListName: list.name}, true)
