 ...
 */
 Repository.prototype.shouldInclude = function(relPath) {
  ...
  try {
    ...
    console.error(`Failed to check ignore status for dir: [${relPath}] in: [${this.path}]. It will be included.`, e)
  }
  return include;
};
...
Repository.prototype.addTaskToFile = function(filePath, list, content, order, cb) {
  ...
  const appendTask = (file, cb) => {
    ...
    if (taskSyntax === Task.Types.MARKDOWN) {
      ...
      const task = newCard({description: description.split(eol.lf), meta: {}, order, text, beforeText: taskPrefix}, this.project)
  ...
  if (file) {
    ...
    const content = journalTemplate && journalTemplate.trim() ? journalTemplate + String(eol.lf) : ''
    file = new File({repoId: this.path, filePath: relPath, content, modifiedTime, createdTime, project: this.project})
    appendTask(file, cb)
  }
}
...
Repository.getTasksByList = function (repo, tasksAry, noSort, populateFiltered) {
  if (!repo) return []
  var tasks = {};
  ...
  lists.forEach((list) => {
    if (list.filter && populateFiltered) return Repository.populateFilteredList(list, tasksAry)
...
Repository.trimFunctionsInSettings = function (config) {
  ...
  if (Array.isArray(links)) {
    ...
    console.log('settings cards.links to:', links)
