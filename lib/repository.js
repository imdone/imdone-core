...
var _                = require('lodash'),
    ...
    rqlToMongo       = require('@swimlane/rql-to-mongo'),
    sift             = require('sift'),
    Task             = require('./task');
...
Repository.convertRqlToMongo = function (queryString) {
  const { RQLToMongo } = rqlToMongo
  queryString = queryString
    .replace(/\//g,'%2F')
    .replace(/:/g,'%3A')
  return RQLToMongo.convertRQLString(queryString)
} 
...
Repository.query = function(tasks, queryString) {
  ...
  try {
    ...
    try {
      query = Repository.convertRqlToMongo(queryString)
    } catch {}
  }
  let result = []
  if (query) result = tasks.filter(sift(query))
  if (!query || result.length === 0) result = Repository.regexQuery(tasks, queryString)
  return result
}
