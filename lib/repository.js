...
Repository.prototype.renameList = function(oldName, newName, cb) {
  ...
  // TODO: Let's use saveModifiedFiles here id:15 gh:100 +fix +fix +fix +fix +fix +fix +fix +fix
...
Repository.prototype.getTasksByList = function(noSort) {
  var tasks = {};
  var allTasks = noSort ? this.getTasks() : _.sortBy(this.getTasks(), ['order', 'text'])
...
Repository.prototype.query = function(queryString) {
  ...
  if (result.length === 0 || result.length === this.getTasks().length) {
    result = this.getTasks().filter(task => task.rawTask.search(new RegExp(tools.escapeRegExp(queryString),'i')) > -1)
  }
  return result
}
