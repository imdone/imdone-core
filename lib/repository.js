...
Repository.prototype.appendTask = function({file, content, list}, cb) {
  ...
  const line = fileContent.split(eol.lf).length - 1
  const taskId = File.getTaskId(file.path, line, text)
  file.setContent(`${fileContent}${appendContent}${"\n".repeat(2)}`)
...
// DOING Write a test to make sure this returns the added task
// [After adding a new card in the editor use the task id to activate the new card](imdone://card.select//Users/jesse/projects/imdone-projects/imdone/imdone-notes?sid=nb0Bp3CVCc40rTurrQ092)
// #appended-task-id
// <!-- order:-20 sid:x8XLsBPyRTuYvfLeLcp_Y -->
