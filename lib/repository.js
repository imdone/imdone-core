...
Repository.prototype.createListeners = function() {
  ...
  this.taskListener = function(event, task) {
    if (!self.listExists(task.list)) {
      const list = new List({name: task.getList()})
...
Repository.prototype.getFiles = function(paths) {
  if (_.isUndefined(paths)) return fastSort(this.files).by({asc: 'path'});
...
Repository.prototype.getFilesWithTasks = function() {
  ...
  returnfastSort(files).by({asc: 'path'});
};
...
Repository.prototype.addTaskToFile = function(filePath, list, content, order, cb) {
  ...
  const taskSyntax = _get(this.config, 'settings.newCardSyntax') || "MARKDOWN"
  const taskPrefix = `${_get(this.config, 'settings.taskPrefix') || "#"} `
  ...
  const appendTask = (file, cb) => {
    ...
    const sep = fileContent.indexOf(crlf) > -1 ? crlf : String(eol.lf)
    const pre = length > 0 ? sep : ''
    const description = lines.length > 1 ? lines.slice(1).join(eol.lf) : ''
    order = order || ''
    const appendContent = (taskSyntax === "MARKDOWN") 
      ? `${pre}${taskPrefix}[${text}](#${list}:${order})\n${description}` 
      : `${pre}${taskPrefix}#${list}:${order} ${content}`
...
Repository.prototype.runFilteredListQuery = function(list, tasks) {
  try {
    list.tasks = Repository.query(tasks, list.filter)
  } catch (e) {
    list.tasks = []
  }
}
...
Repository.prototype.getTasksInList = function(name, offset, limit) {
  ...
  var allTasks = fastSort(_.where(tasks, {list:name})).by(DEFAULT_SORT);
...
Repository.convertRqlToMongo = function (queryString) {
  queryString = queryString
    .replace(/\//g,'%2F')
    .replace(/:/g,'%3A')
  const mongoQuery = mongoRql(queryString)
  return mongoQuery
} 
...
Repository.query = function(tasks, queryString) {
  ...
  try {
    ...
    try {
      const mongoQuery = Repository.convertRqlToMongo(queryString)
      query = mongoQuery.criteria
      sort = Repository.parseSortFromMongoQuery(mongoQuery)
    } catch (e) {
      log(`Unable to parse ${queryString} using rql`)
    }
  }
  let result = []
  ...
  result = fastSort(result).by(sort)
  return result
}
...
Repository.trimFunctionsInSettings = function (config) {
  ...
  if (_.isObject(computed)) {
    Object.keys(computed).forEach(key => {
      ...
      if (_.isFunction(value)) {
        const functionMatch = value.toString().match(functionRegex())
        ...
        try {
          ...
          console.error('Unable to parse computed function:', value.toString())
          console.error(e)
        }
  ...
  const links = _get(config, 'settings.cards.links')
  if (_.isArray(links)) {
    links.map(link => {
      if (link.action && _.isFunction(link.action)) {
        const functionMatch = link.action.toString().match(functionRegex())
        const lines = eol.split(functionMatch[3]);
        const funcBody = lines.filter(line => line.trim().length > 0).join(eol.lf)
        let func = null
        try {
          ...
          console.error('Unable to parse link action function:', value.toString())
          console.error(e)
        }
        if (func) link.action = func
      }
      return link
    })
