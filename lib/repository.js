...
Repository.trimFunctionsInSettings = function (config) {
  ...
  let cardExports = _get(config, 'settings.exports')
  if (_.isArray(cardExports)) {
    cardExports = cardExports.map(cardExport => {
      const propValue = cardExport.function
      const func = trimFunction(propValue)
      if (func) cardExport.function = func
      return cardExport
    })
    config.settings.exports = cardExports
  }
}
