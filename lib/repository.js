...
// [modifyTask must be async.  Get file content before proceeding.](#doing:20)
/**
 * Description
 * @method modifyTask
 * @param {} task
...
Repository.prototype.modifyTask = function(task, cb) {
  ...
  if (!file.isModified()) this.readFileContent(file, function(err, file) {
    if (err) return cb(err);
    cb(null, file.modifyTask(task, this.getConfig()));
  });
};
