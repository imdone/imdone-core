...
Repository.prototype.runFilteredListQuery = function(list, tasks) {
  try {
    list.tasks = Repository.query(tasks, list.filter).map(task => {
      const filteredListTask = new Task(tis.config, {...task, filteredListName: list.name})
...
Repository.deserialize = function(repo, cb) {
  ...
  var newRepo = new Repository(repo.path, new Config(repo.config));
  var count = 0;
  repo.files.forEach(file => {
    log("file:%j", file);
    newRepo.addFile(new File({file: file, config: newRepo.config, languages: languages}), function(err) {
      if (err) return cb(err);
      count++;
