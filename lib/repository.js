...
Repository.regexQuery = function (tasks, queryString) {
  return tasks.map((task) => {
    const match = task.content.match(new RegExp(tools.escapeRegExp(queryString), 'i'))
    task.match = match
    if (match && queryString.trim() && task.interpretedContent) {
      task.interpretedContent = task.interpretedContent.replace(new RegExp(`(${tools.escapeRegExp(queryString.trim())})`, 'gi'), '==$1==' )
