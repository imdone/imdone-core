...
class Config {
  constructor (opts) {
    ...
    try {
      const path = _path.join(this.path, '.imdone', 'actions', 'card')
      delete require.cache[require.resolve(path)]
      this.cardModuleActions = require(path)
    } catch (e) {
      console.warn(`No card actions module found for ${this.path}`)
      this.cardModuleActions = []
    }
    try {
      const path = _path.join(this.path, '.imdone', 'actions', 'board')
      delete require.cache[require.resolve(path)]
      this.boardModuleActions = require(path).map(({title, action}) => {
        return {name: title, action}
      })
    } catch (e) {
      console.warn(`No board actions module found for ${this.path}`)
      this.boardModuleActions = []
    }
  }
  ...
  get cardActions () {
    return [...this.cardModuleActions, ..._get(this, 'settings.cards.links', [])]
  }
  ...
  get boardActions () {
    ...
    return [...this.boardModuleActions, ...settingsActions]
  }
