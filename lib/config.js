...
class Config {
  constructor(opts) {
    ...
    if (this.path) {
      ...
      this.configDir = _path.join(this.path, '.imdone')

      this.cardModuleActions = this.loadExtensionModule([], 'actions', 'card')
      this.boardModuleActions = this.loadExtensionModule(
        [],
        'actions',
        'board'
      ).map(({ title, action }) => ({ name: title, action }))
  ...
  get cardActions() {
    return [
      ...
      ..._get(this, 'settings.cards.links', []),
    ]
  }
