...
export class Config {
  ...
  set fields(fields) {
    if (!this.settings.cards) this.settings.cards = {}
    this.settings.cards.fields = fields
  }
  ...
  set archiveCompleted(archive) {
    this.settings.cards.archiveCompleted = archive
  }
  ...
  get metaSep() {
    return this?.cards?.metaSep ?? COLON
  }

  set metaSep(value) {
    if (!META_SEPS.includes(value)) {
      throw new Error(`Invalid meta separator: ${value}`)
    }
    this.settings.cards.metaSep = value
  }
