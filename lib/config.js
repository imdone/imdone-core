...
Config.load = async (imdonePath) => {
  const configPath = _path.join(imdonePath, CONFIG_FILE_YML)
  const configLike = await Config.getYamlConfig(configPath)
  return new Config(configLike)
}
