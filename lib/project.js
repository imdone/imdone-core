...
module.exports = class WorkerProject extends Project {
  ...
  async addTaskToFile({path, list, content, tags = [], contexts = [], meta = [], addTemplate}) {
    ...
    content = this.addOrderMetaToContent(content, list)
    ...
    return new Promise((resolve, reject) => {
      this.repo.addTaskToFile(filePath, list, content, (err, file) => {
        if (err) return reject(err)
        // TODO Move the card to the correct position based on addNewCardsToTop
        // #appended-task-id
        // <!-- order:-65 sid:OdhrAwat2CuWO1b6Obdeh -->
        
        // TODO Resolve with the file and taskId
        ...
        // #appended-task-id
        // <!-- order:-65 sid:9yRoRHlLd0tPnQVcd9pgi -->
        resolve(file)
      })
    })
  }

  addOrderMetaToContent(content, _list) {
    const list = this.lists.find(l => l.name === _list)
    let order
    if (this.config.isAddNewCardsToTop()) {
      order = list.tasks.length
        ? list.tasks[0].order - 1
        : 0
    } else {
      order = list.tasks.length
        ? list.tasks[list.tasks.length - 1].order + 1
        : 0
    }
    return Task.addToLastCommentInContent(content, `order${this.config.getMetaSep()}${order}`, this.config.isMetaNewLine())
  }
