...
export class WorkerProject extends Project {
  ...
  */
  ...
  async addTaskToFile({path, list = this.config?.getDefaultList(), content, tags = [], contexts = [], meta = [], useCardTemplate = false}) {
    logger.debug(`[TRACE] addTaskToFile START - path: ${path}, contentLength: ${content?.length || 0}`)
    ...
    logger.debug(`[TRACE] addTaskToFile - after onBeforeAddTask, contentLength: ${content?.length || 0}`)
    ...
    logger.debug(`[TRACE] addTaskToFile - after newCard, contentLength: ${content?.length || 0}`)

    const boardData = await this.pluginManager.getBoardProperties();
    const data = { ...boardData, ...cardData}
    logger.debug(`[TRACE] addTaskToFile - about to call format(), contentLength: ${content?.length || 0}`)
    const formatStart = Date.now()
    content = format(content, data)
    const formatTime = Date.now() - formatStart
    logger.debug(`[TRACE] addTaskToFile - format() completed in ${formatTime}ms, contentLength: ${content?.length || 0}`)
    ...
    logger.debug(`[TRACE] addTaskToFile - about to call repo.addTaskToFile, contentLength: ${content?.length || 0}`)
