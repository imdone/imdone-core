...
import { resolveDefaultFilteredMovePosition } from './default-filtered-move-position.js';
...
export class WorkerProject extends Project {
  ...
  async moveTaskInDefaultFilteredCards(task, newList, newPos) {
    const repositoryPosition = resolveDefaultFilteredMovePosition({
      task,
      newList,
      newPosition: newPos,
      filteredCards: this.getDefaultFilteredCards(),
      repositoryListCards: this.repo.getTasksInList(newList),
    })
    return await this.repo.moveTask({task, newList, newPos: repositoryPosition})
  }
  ...
  */
  ...
  async addTaskToFile({path, list = this.config?.getDefaultList(), content, tags = [], contexts = [], meta = [], useCardTemplate = false}) {
    logger.debug(`[INSTRUMENT] addTaskToFile START - path: ${path}, contentLength: ${content?.length || 0}`)
    ...
    logger.debug(`[INSTRUMENT] addTaskToFile - after onBeforeAddTask, contentLength: ${content?.length || 0}`)
    ...
    logger.debug(`[INSTRUMENT] addTaskToFile - after newCard, contentLength: ${content?.length || 0}`)

    const boardData = await this.pluginManager.getBoardProperties();
    const data = { ...boardData, ...cardData}
    logger.debug(`[INSTRUMENT] addTaskToFile - about to call format(), contentLength: ${content?.length || 0}`)
    const formatStart = Date.now()
    content = format(content, data)
    const formatTime = Date.now() - formatStart
    logger.debug(`[INSTRUMENT] addTaskToFile - format() completed in ${formatTime}ms, contentLength: ${content?.length || 0}`)
    ...
    logger.debug(`[INSTRUMENT] addTaskToFile - about to call repo.addTaskToFile, contentLength: ${content?.length || 0}`)
