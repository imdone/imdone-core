...
module.exports = class WorkerProject extends Project {
  ...
  */
  newCard({ list, path, template, title, comments, emit = true }) {
    if (!path) path = this.getFullPath(this.getNewCardsFile({title}))

    let stat = fileGateway.statSync(path)
    if (!stat) {
      const dir = _path.dirname(path)
      fileGateway.mkdirpSync(dir)
      if (dir !== path) fileGateway.writeFileSync(path, '')
      stat = fileGateway.statSync(path)
    }
