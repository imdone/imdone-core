...
Project.prototype.emitModified = function(data) {
  var self = this;
  data = _.extend({project:self.name}, data || {});
  this.emit('project.modified', data);
};
...
Project.prototype.createListeners = function() {
  ...
  // DOING:10 Add file.update anf file.delete listener for repos and emit project.modified
  this.fileUpdateListener = function(file) {
    self.emit('file.update', file);
  };

  this.fileDeleteListener = function(file) {
    self.emit('file.delete', {project:self.getName()});
  };
...
Project.prototype.addRepo = function(repo, cb) {
  ...
  if (this.getRepo(repo.getId()) === undefined) {
    ...
    this.initRepo(repo, function() {
      ...
      self.emitModified({repoAdded:{repoId:repo.getId()}});
    });
  } else cb();
};
...
Project.prototype.moveList = function(name, pos, cb) {
  ...
  if (list) {
    ...
    this.storeLists(lists, function(err, result) {
      if (!err) self.emitModified({listMoved:{name:name, pos:pos}});
      self.busy = false;
      cb(err, result);
    });
  } else {
...
Project.prototype.renameList = function(oldName, newName, cb) {
  ...
  if (funcs.length > 0) {
    async.parallel(funcs, function(err, results) {
      if (!err) self.emitModified({listRenamed:{oldName:oldName, newName:newName}});
      self.busy = false;
      cb(err, results);
    });
  } else {
...
Project.prototype.moveTasks = function(tasks, newList, newPos, cb) {
  ...
  if (funcs.length > 0) {
    async.parallel(funcs, function(err, results) {
      if (!err) self.emitModified({tasksMoved:{files:filesToSave}});
      self.busy = false;
      cb(err, results);
    });
  } else {
...
Project.prototype.hideList = function(name, cb) {
  ...
  if (list) {
    ...
    this.storeLists(this.getLists(), function() {
      self.emitModified({listHidden:name});
      cb();
    });
...
Project.prototype.showList = function(name, cb) {
  ...
  if (list) {
    list.hidden = false;
    this.storeLists(this.getLists(), function() {
      self.emitModified({listShown:name});
      cb();
    });
  } else cb("List does not exist.");
};
...
Project.prototype.removeList = function(name, cb) {
  ...
  if (list) {
    this.storeLists(_.reject(this.getLists(), {name:name}), function() {
      self.emitModified({listRemoved:name});
      cb();
    });
  } else cb("List does not exist.");
};
