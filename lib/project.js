...
module.exports = class WorkerProject extends Project {
  ...
  toJSON () {
    ...
    console.time('plugin updateLists')
    this.pluginManager.updateLists(allLists)
    console.timeEnd('plugin updateLists')
  ...
  init (cb = () => {}) {
    ...
    this.pluginManager.on('plugin-installed', ({name, version}) => {
      this.emitUpdate()
    })
    this.pluginManager.on('plugins-reloaded', () => {
