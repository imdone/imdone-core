...
module.exports = class WorkerProject extends Project {
  ...
  toImdoneJSON() {
    ...
    return {
      ...
      totals,
      filter: this.filter,
      defaultFilter: this.defaultViewFilter,
  ...
  get filteredCards() {
    ...
    return this.getCards(this.filter, cards)
  }

  getLists(opts) {
    const {tasks = this.getCards(), populateFiltered = false} = opts || {}
  ...
  getCards(filter = this.defaultViewFilter, tasks = this.repo.getTasks()) {
    if (!filter) filter = this.defaultViewFilter
