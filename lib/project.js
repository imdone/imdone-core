...
module.exports = class WorkerProject extends Project {
  ...
  init(cb) {
    const promise = new Promise(async (resolve, reject) => {
      ...
      this.data = this.pluginManager.getBoardProperties()
  ...
  async toImdoneJSON() {
    ...
    const allLists = this.getListsForImdoneJSON(totals)
    ...
    const data = this.data = this.pluginManager.getBoardProperties()
    ...
    await this.pluginManager.onBoardUpdate(
      this.getLists({
        tasks: cards,
      })
    )
  ...
  getCards(filter, tasks = this.getDefaultFilteredCards()) {
    tasks = filter ? this.filterCards(tasks, filter) : tasks
    return tasks
  }

  filterCards(tasks, filter) {
    const {
      result,
    } = Repository.filterCards(tasks, filter)
    return result
  }
