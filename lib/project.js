...
module.exports = class WorkerProject extends Project {
  ...
  newCard(opts = {}) {
    let { list, path, template, title } = opts
    if (path) {
    ...
    this.emit('project.newCard', data)
    return data
  }

  async addTaskToFile({path, list, content, tags, contexts}) {
    const cardData = this.newCard({list, path, title: eol.split(content)[0]})
    const filePath = cardData.path
    if (tags) {
      let tagContent = ''
      tags.forEach(tag => {
        tagContent = `${tagContent} ${this.config.getTagPrefix()}${tag}`
      })
      content = Task.addToLastCommentInContent(content, tagContent, this.config.isMetaNewLine())
    }
    if (contexts) {
      let contextContent = ''
      ...
      content = Task.addToLastCommentInContent(content, contextContent, this.config.isMetaNewLine())
    }
    const cardContent = content + cardData.template
    return new Promise((resolve, reject) => {
      this.repo.addTaskToFile(filePath, list, cardContent, (err, file) => {
        if (err) return reject(err)
        resolve(file)
      })
    })
  ...
  sanitizeFileName(name) {
    let fileName = sanitize(removeMD(name))
